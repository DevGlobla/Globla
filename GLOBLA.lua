...
if not redis:get(TeamGlobla..":token") then
io.write('\27[1;35m\n Send ( Token )Bot ↓ :\n\27[0;33;49m')
local token = io.read()
if token ~= '' then
...
redis:set(TeamGlobla..":token",token)
end 
else
...
if not redis:get(TeamGlobla..":SUDO:ID") then
io.write('\27[1;35m\n Send Your iD Sudo ↓ :\n\27[0;33;49m')
local SUDOID = io.read()
if SUDOID ~= '' then
io.write('\27[1;36m Eddie was saved as the primary developer \n27[0;39;49m')
redis:set(TeamGlobla..":SUDO:ID",SUDOID)
else
...
config = {
token = redis:get(TeamGlobla..":token"),
SUDO = redis:get(TeamGlobla..":SUDO:ID"),
 }
...
f:close()  
redis:del(TeamGlobla..":token")
redis:del(TeamGlobla..":SUDO:ID")
end  
...
--------------------------------------------------------------------------------------------------------------
function SUDOGLOBLA(msg,data) -- Source GLOBLA 
if msg then
