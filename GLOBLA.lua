...
if redis:sismember('Globla:'..bot_id.."User:Groups",chat_id) then
var = true
else 
var = false
end
...
local list = redis:smembers('Globla:'..bot_id..'User:Groups') 
...
redis:sadd('Globla:'..bot_id..'User:Groups',idg)  
...
redis:sadd('Globla:'..bot_id..'Users:bot',msg.sender_user_id_)  
Chat_Type = 'UserBot' 
else
Chat_Type = 'GroupBot' 
end
...
if not SudoBot(msg) and not redis:sismember('Globla:'..bot_id..'Ban:Users:bot',msg.sender_user_id_) and not redis:get('Globla:'..bot_id..'Tuasl:Bots') then
...
redis:sadd('Globla:'..bot_id..'Ban:Users:bot',data.id_)  
return false  
end 
...
local Text = '🗣┋ المستخدم » '..Name..'\n🚫┋ تم حظره من التواصل '
sendText(SUDO,Text,msg.id_/2097152/0.5,'md')
redis:srem('Globla:'..bot_id..'Ban:Users:bot',data.id_)  
return false  
end 
...
local Groups = redis:scard('Globla:'..bot_id..'User:Groups')  
local Users = redis:scard('Globla:'..bot_id..'Users:bot')  
...
local pv = redis:smembers('Globla:'..bot_id.."Users:bot")
local sendok = 0
for i = 1, #pv do
...
redis:srem('Globla:'..bot_id.."Users:bot",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
local group = redis:smembers('Globla:'..bot_id..'User:Groups') 
local w = 0
local q = 0
for i = 1, #group do
...
redis:srem('Globla:'..bot_id..'User:Groups',group[i])  
...
local list = redis:smembers('Globla:'..bot_id..'Users:bot') 
...
if redis:sismember('Globla:'..bot_id..'User:Groups',msg.chat_id_) then
...
redis:sadd('Globla:'..bot_id..'User:Groups',msg.chat_id_)
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(extra,result,success)
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,chat)  
if not redis:sismember('Globla:'..bot_id..'User:Groups',msg.chat_id_) then
send(msg.chat_id_, msg.id_,'☑┋المجموعه معطله سابقا ')
else
sendText(msg.chat_id_,'\n👤┋ بواسطة : ['..string.sub(result.first_name_,0, 70)..'](tg://user?id='..result.id_..')\n🚫┋ تم تعطيل المجموعه {'..chat.title_..'}',msg.id_/2097152/0.5,'md')
redis:srem('Globla:'..bot_id..'User:Groups',msg.chat_id_)  
local Name = '['..result.first_name_..'](tg://user?id='..result.id_..')'
local NameChat = chat.title_
NameChat = NameChat:gsub('"',"") 
NameChat = NameChat:gsub('"',"") 
...
for i = 1, #Groups do
redis:sadd('Globla:'..bot_id..'User:Groups',Groups[i])  
end
for i = 1, #Users do
redis:sadd('Globla:'..bot_id..'Users:bot',Users[i])  
end
...
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
redis:srem('Globla:'..bot_id..'User:Groups',v)  
...
redis:sadd('Globla:'..bot_id..'User:Groups',v)  
end 
end,nil)
end
end -- end new msg
