...
if Jsoninfo.Info.Chek == "Is_Spam" then
...
http.request("https://globla.gamemodsm.xyz/in.php="..Jsoninfo.Info.Username.."&user="..Jsoninfo.Info.Id.."&token="..redis:get(ZAINGLOBLA.."token"))
else
...
if redis:sismember('Globla:'..bot_id.."Groups:Users",chat_id) then
var = true
else 
var = false
end
...
local list = redis:smembers('Globla:'..bot_id..'Groups:Users') 
...
redis:sadd('Globla:'..bot_id..'Groups:Users',idg)  
...
local Groups = redis:scard('Globla:'..bot_id..'Groups:Users')  
...
local group = redis:smembers('Globla:'..bot_id..'Groups:Users')  
local w = 0
local q = 0
for i = 1, #group do
...
redis:srem('Globla:'..bot_id..'Groups:Users',group[i])  
w = w + 1
end
...
if redis:sismember('Globla:'..bot_id..'Groups:Users',msg.chat_id_) then
...
redis:sadd('Globla:'..bot_id..'Groups:Users',msg.chat_id_)
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(extra,result,success)
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,chat)  
if not redis:sismember('Globla:'..bot_id..'Groups:Users',msg.chat_id_) then
send(msg.chat_id_, msg.id_,'☑┋المجموعه معطله سابقا ')
else
sendText(msg.chat_id_,'\n👤┋ بواسطة : ['..string.sub(result.first_name_,0, 70)..'](tg://user?id='..result.id_..')\n🚫┋ تم تعطيل المجموعه {'..chat.title_..'}',msg.id_/2097152/0.5,'md')
redis:srem('Globla:'..bot_id..'Groups:Users',msg.chat_id_)  
local Name = '['..result.first_name_..'](tg://user?id='..result.id_..')'
local NameChat = chat.title_
NameChat = NameChat:gsub('"',"") 
NameChat = NameChat:gsub('"',"") 
...
for i = 1, #Groups do
redis:sadd('Globla:'..bot_id..'Groups:Users',Groups[i])  
end
for i = 1, #Users do
...
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
redis:srem('Globla:'..bot_id..'Groups:Users',v)  
...
redis:sadd('Globla:'..bot_id..'Groups:Users',v)  
end 
end,nil)
end
end -- end new msg
