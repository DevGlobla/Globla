...
local GLOBLA = false  
if tonumber(msg.sendMessageer_user_id_) == tonumber(SUDO) then
GLOBLA = true  
end
return GLOBLA  
end 
function Sudo(msg) 
local hash = redis:sismember('Globla:'..bot_id..'Sudo:User', msg.sendMessageer_user_id_) 
...
local hash = redis:sismember('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_, msg.sendMessageer_user_id_) 
...
local hash = redis:sismember('Globla:'..bot_id..'Constructor'..msg.chat_id_, msg.sendMessageer_user_id_) 
...
local hash = redis:sismember('Globla:'..bot_id..'Manager'..msg.chat_id_,msg.sendMessageer_user_id_)    
...
function Mod(msg)
local hash = redis:sismember('Globla:'..bot_id..'Mod:User'..msg.chat_id_,msg.sendMessageer_user_id_)    
...
local hash = redis:sismember('Globla:'..bot_id..'Special:User'..msg.chat_id_,msg.sendMessageer_user_id_) 
...
var = true  
elseif redis:sismember('Globla:'..bot_id..'Sudo:User', user_id) then
var = true  
elseif redis:sismember('Globla:'..bot_id..'Basic:Constructor'..chat_id, user_id) then
var = true
elseif redis:sismember('Globla:'..bot_id..'Constructor'..chat_id, user_id) then
var = true  
elseif redis:sismember('Globla:'..bot_id..'Manager'..chat_id, user_id) then
var = true  
elseif redis:sismember('Globla:'..bot_id..'Mod:User'..chat_id, user_id) then
var = true  
elseif redis:sismember('Globla:'..bot_id..'Special:User'..chat_id, user_id) then  
var = true  
else  
var = false  
end  
...
var = 'البوت'
elseif redis:sismember('Globla:'..bot_id..'Sudo:User', user_id) then
var = redis:get('Globla:'..bot_id.."Sudo:Rd"..msg.chat_id_) or 'المطور'  
elseif redis:sismember('Globla:'..bot_id..'Basic:Constructor'..chat_id, user_id) then
var = redis:get('Globla:'..bot_id.."BasicConstructor:Rd"..msg.chat_id_) or 'المنشئ اساسي'
elseif redis:sismember('Globla:'..bot_id..'Constructor'..chat_id, user_id) then
var = redis:get('Globla:'..bot_id.."Constructor:Rd"..msg.chat_id_) or 'المنشئ'  
elseif redis:sismember('Globla:'..bot_id..'Manager'..chat_id, user_id) then
var = redis:get('Globla:'..bot_id.."Manager:Rd"..msg.chat_id_) or 'المدير'  
elseif redis:sismember('Globla:'..bot_id..'Mod:User'..chat_id, user_id) then
var = redis:get('Globla:'..bot_id.."Mod:Rd"..msg.chat_id_) or 'الادمن'  
elseif redis:sismember('Globla:'..bot_id..'Special:User'..chat_id, user_id) then  
var = redis:get('Globla:'..bot_id.."Special:Rd"..msg.chat_id_) or 'المميز'  
else  
var = redis:get('Globla:'..bot_id.."Memp:Rd"..msg.chat_id_) or 'العضو'
end  
return var
end 
function ChekAdd(chat_id)
if redis:sismember('Globla:'..bot_id.."Chek:Groups",chat_id) then
var = true
else 
var = false
end
...
local var = true
if redis:get('Globla:'..bot_id..'add:ch:id') then
local url , res = https.request("https://api.telegram.org/bot"..token.."/getchatmember?chat_id="..redis:get('Globla:'..bot_id..'add:ch:id').."&user_id="..User);
...
local function sendMessage(chat_id, reply_to_message_id, text)
...
local function sendMessageText(chat_id, text, reply_to_message_id, markdown) 
sendMessage_api = "https://api.telegram.org/bot"..token local url = sendMessage_api..'/sendMessageMessage?chat_id=' .. chat_id .. '&text=' .. URL.escape(text) if reply_to_message_id ~= 0 then url = url .. '&reply_to_message_id=' .. reply_to_message_id  end if markdown == 'md' or markdown == 'markdown' then url = url..'&parse_mode=Markdown' elseif markdown == 'html' then url = url..'&parse_mode=HTML' end return s_api(url)  
end
function sendMessage_inline_key(chat_id,text,keyboard,inline,reply_id) 
local response = {} response.keyboard = keyboard response.inline_keyboard = inline response.resize_keyboard = true response.one_time_keyboard = false response.selective = false  local sendMessage_api = "https://api.telegram.org/bot"..token.."/sendMessageMessage?chat_id="..chat_id.."&text="..URL.escape(text).."&parse_mode=Markdown&disable_web_page_preview=true&reply_markup="..URL.escape(JSON.encode(response)) if reply_id then sendMessage_api = sendMessage_api.."&reply_to_message_id="..reply_id end return s_api(sendMessage_api) 
end
local function GetInputFile(file)  
local file = file or ""   if file:match('/') then  infile = {ID= "InputFileLocal", path_  = file}  elseif file:match('^%d+$') then  infile = {ID= "InputFileId", id_ = file}  else  infile = {ID= "InputFilePersistentId", persistent_id_ = file}  end return infile 
end
local function sendMessageRequest(request_id, chat_id, reply_to_message_id, disable_notification, from_background, reply_markup, input_message_content, callback, extra) 
tdcli_function ({  ID = request_id,    chat_id_ = chat_id,    reply_to_message_id_ = reply_to_message_id,    disable_notification_ = disable_notification,    from_background_ = from_background,    reply_markup_ = reply_markup,    input_message_content_ = input_message_content,}, callback or dl_cb, extra) 
end
local function sendMessageAudio(chat_id,reply_id,audio,title,caption)  
tdcli_function({ID="SendMessage",  chat_id_ = chat_id,  reply_to_message_id_ = reply_id,  disable_notification_ = 0,  from_background_ = 1,  reply_markup_ = nil,  input_message_content_ = {  ID="InputMessageAudio",  audio_ = GetInputFile(audio),  duration_ = '',  title_ = title or '',  performer_ = '',  caption_ = caption or ''  }},dl_cb,nil)
end  
local function sendMessageVideo(chat_id, reply_to_message_id, disable_notification, from_background, reply_markup, video, duration, width, height, caption, cb, cmd)    
local input_message_content = { ID = "InputMessageVideo",      video_ = getInputFile(video),      added_sticker_file_ids_ = {},      duration_ = duration or 0,      width_ = width or 0,      height_ = height or 0,      caption_ = caption    }    sendMessageRequest('SendMessage', chat_id, reply_to_message_id, disable_notification, from_background, reply_markup, input_message_content, cb, cmd)  
end
function sendMessageDocument(chat_id, reply_to_message_id, disable_notification, from_background, reply_markup, document, caption, dl_cb, cmd) 
tdcli_function ({ID = "SendMessage",chat_id_ = chat_id,reply_to_message_id_ = reply_to_message_id,disable_notification_ = disable_notification,from_background_ = from_background,reply_markup_ = reply_markup,input_message_content_ = {ID = "InputMessageDocument",document_ = getInputFile(document),caption_ = caption},}, dl_cb, cmd) 
end
local function sendMessageVoice(chat_id, reply_to_message_id, disable_notification, from_background, reply_markup, voice, duration, waveform, caption, cb, cmd)  
local input_message_content = {   ID = "InputMessageVoice",   voice_ = getInputFile(voice),  duration_ = duration or 0,   waveform_ = waveform,    caption_ = caption  }  sendMessageRequest('SendMessage', chat_id, reply_to_message_id, disable_notification, from_background, reply_markup, input_message_content, cb, cmd) 
end
local function sendMessageSticker(chat_id, reply_to_message_id, disable_notification, from_background, reply_markup, sticker, cb, cmd)  
local input_message_content = {    ID = "InputMessageSticker",   sticker_ = getInputFile(sticker),    width_ = 0,    height_ = 0  } sendMessageRequest('SendMessage', chat_id, reply_to_message_id, disable_notification, from_background, reply_markup, input_message_content, cb, cmd) 
end
local function sendMessagePhoto(chat_id, reply_to_message_id, disable_notification, from_background, reply_markup, photo,caption)   
...
local list = redis:smembers('Globla:'..bot_id..'Chek:Groups') 
local t = '{"BOT_ID": '..bot_id..',"GP_BOT":{'  
for k,v in pairs(list) do   
NAME = 'GLOBLA Chat'
link = redis:get('Globla:'..bot_id.."Private:Group:Link"..msg.chat_id_) or ''
ASAS = redis:smembers('Globla:'..bot_id..'Basic:Constructor'..v)
MNSH = redis:smembers('Globla:'..bot_id..'Constructor'..v)
MDER = redis:smembers('Globla:'..bot_id..'Manager'..v)
MOD = redis:smembers('Globla:'..bot_id..'Mod:User'..v)
if k == 1 then
t = t..'"'..v..'":{"GLOBLA":"'..NAME..'",'
else
t = t..',"'..v..'":{"GLOBLA":"'..NAME..'",'
...
File:write(t)
File:close()
sendMessageDocument(msg.chat_id_, msg.id_,0, 1, nil, './Offices/'..bot_id..'.json', '📋┋ عدد مجموعات التي في البوت { '..#list..'}')
end
...
sendMessage(chat,msg.id_,"📋┋ ملف النسخه الاحتياطيه ليس لهذا البوت")   
return false 
end      
local File = json:decode(https.request('https://api.telegram.org/bot'.. token..'/getfile?file_id='..ID_FILE) ) 
download_to_file('https://api.telegram.org/file/bot'..token..'/'..File.result.file_path, ''..File_Name) 
sendMessage(chat,msg.id_,"♻┋ جاري ...\n📥┋ رفع الملف الان")   
else
sendMessage(chat,msg.id_,"*📛┋ عذرا الملف ليس بصيغة {JSON} يرجى رفع الملف الصحيح*")   
end      
local info_file = io.open('./'..bot_id..'.json', "r"):read('*a')
local groups = JSON.decode(info_file)
for idg,v in pairs(groups.GP_BOT) do
redis:sadd('Globla:'..bot_id..'Chek:Groups',idg)  
...
redis:sadd('Globla:'..bot_id..'Constructor'..idg,idmsh)
end
end
if v.MDER then
for k,idmder in pairs(v.MDER) do
redis:sadd('Globla:'..bot_id..'Manager'..idg,idmder)  
end
end
if v.MOD then
for k,idmod in pairs(v.MOD) do
redis:sadd('Globla:'..bot_id..'Mod:User'..idg,idmod)  
end
end
if v.ASAS then
for k,idASAS in pairs(v.ASAS) do
redis:sadd('Globla:'..bot_id..'Basic:Constructor'..idg,idASAS)  
end
end
end
sendMessage(chat,msg.id_,"\n🗂┋تم رفع الملف بنجاح وتفعيل المجموعات\n☑┋ورفع {الامنشئين الاساسين ; والمنشئين ; والمدراء; والادمنيه} بنجاح")   
end
local function trigger_anti_spam(msg,type)
tdcli_function ({ID = "GetUser",user_id_ = msg.sendMessageer_user_id_},function(arg,data)
local Name = '['..utf8.sub(data.first_name_,0,40)..'](tg://user?id='..data.id_..')'
if type == 'kick' then 
Text = '\n👤┋ العضــو » '..Name..'\n🚫┋ قام بالتكرار هنا وتم طرده '  
sendMessageText(msg.chat_id_,Text,0,'md')
Group_Kick(msg.chat_id_,msg.sendMessageer_user_id_) 
my_ide = msg.sendMessageer_user_id_
msgm = msg.id_
local num = 100
for i=1,tonumber(num) do
tdcli_function ({ID = "GetMessages",chat_id_ = msg.chat_id_,message_ids_ = {[0] = msgm}},function(arg,data) 
if data.messages_[0] ~= false then
if tonumber(my_ide) == (data.messages_[0].sendMessageer_user_id_) then
...
if type == 'keed' then
https.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" ..msg.chat_id_.. "&user_id=" ..msg.sendMessageer_user_id_.."") 
redis:sadd('Globla:'..bot_id..'Muted:User'..msg.chat_id_,msg.sendMessageer_user_id_) 
msgm = msg.id_
my_ide = msg.sendMessageer_user_id_
local num = 100
for i=1,tonumber(num) do
...
redis:incr('Globla:'..bot_id..'Msg_User'..msg.chat_id_..':'..msg.sendMessageer_user_id_) 
Chat_Type = 'GroupBot' 
elseif id:match("^(%d+)") then
redis:sadd('Globla:'..bot_id..'UsersBot',msg.sendMessageer_user_id_)  
Chat_Type = 'UserBot' 
else
Chat_Type = 'GroupBot' 
end
...
if text == '/start' then  
if AddChannel(msg.sendMessageer_user_id_) == false then
local textchuser = redis:get('Globla:'..bot_id..'text:ch:user')
if textchuser then
sendMessage(msg.chat_id_, msg.id_,'['..textchuser..']')
else
sendMessage(msg.chat_id_, msg.id_,'🔅┋عذراً عليك الاشتراك في القناة\n📌┋اشترك هنا ['..redis:get('Globla:'..bot_id..'add:ch:username')..']')
end
return false
end
if SudoBot(msg) then
local bl = '👨🏻‍💻┋ آهلا عزيزي آلمـطـور 🛠\n👨‍🎓┋ آنت آلمـطـور آلآسـآسـي هنا 🗣\n...\n\n🚸┋ تسـتطـيع‏‏ آلتحگم بگل آلآوآمـر الموجودة بالگيبورد\n⚖️┋ فقط آضـغط ع آلآمـر آلذي تريد تنفيذه'
local keyboard = {
{'ضع اسم للبوت ©'},
...
sendMessage_inline_key(msg.chat_id_,bl,keyboard)
else
if not redis:get('Globla:'..bot_id..'Start:Time'..msg.sendMessageer_user_id_) then
...
sendMessage(msg.chat_id_, msg.id_, KleshaStart) 
end
end
redis:setex('Globla:'..bot_id..'Start:Time'..msg.sendMessageer_user_id_,300,true)
return false
end
if not SudoBot(msg) and not redis:sismember('Globla:'..bot_id..'Ban:UsersBot',msg.sendMessageer_user_id_) and not redis:get('Globla:'..bot_id..'Twal:Bot') then
sendMessage(msg.sendMessageer_user_id_, msg.id_, '👨‍💻┋ تم ارسال رسالتك الى المطور')    
tdcli_function ({ID = "ForwardMessages", chat_id_ = SUDO,    from_chat_id_ = msg.sendMessageer_user_id_,    message_ids_ = {[0] = msg.id_},    disable_notification_ = 1,    from_background_ = 1 },function(arg,data) 
tdcli_function ({ID = "GetUser",user_id_ = msg.sendMessageer_user_id_},function(arg,ta) 
vardump(data)
if data and data.messages_[0].content_.sticker_ then
local Name = '['..string.sub(ta.first_name_,0, 40)..'](tg://user?id='..ta.id_..')'
local Text = '📥┋ تم ارسال الملصق من ↓\n - '..Name
sendMessageText(SUDO,Text,0,'md')
end 
end,nil) 
end,nil)
end
...
if result.forward_info_.sendMessageer_user_id_ then     
id_user = result.forward_info_.sendMessageer_user_id_    
end     
tdcli_function ({ID = "GetUser",user_id_ = id_user},function(arg,data) 
if text == 'حظر' then
local Name = '['..string.sub(data.first_name_,0, 40)..'](tg://user?id='..id_user..')'
local Text = '🗣┋ المستخدم » '..Name..'\n🚫┋ تم حظره من التواصل '
sendMessageText(SUDO,Text,msg.id_/2097152/0.5,'md')
redis:sadd('Globla:'..bot_id..'Ban:UsersBot',data.id_)  
return false  
end 
...
local Text = '🗣┋ المستخدم » '..Name..'\n🚫┋ تم حظره من التواصل '
sendMessageText(SUDO,Text,msg.id_/2097152/0.5,'md')
redis:srem('Globla:'..bot_id..'Ban:UsersBot',data.id_)  
return false  
end 
...
sendMessage(msg.chat_id_, msg.id_,GLOBLA_Msg) 
return false  
end 
if text then    
sendMessage(id_user,msg.id_,text)    
local Name = '['..string.sub(data.first_name_,0, 40)..'](tg://user?id='..id_user..')'
...
sendMessageSticker(id_user, msg.id_, 0, 1, nil, msg.content_.sticker_.sticker_.persistent_id_)   
local Name = '['..string.sub(data.first_name_,0, 40)..'](tg://user?id='..id_user..')'
local Text = '🗣┋ المستخدم » '..Name..'\n☑┋ تم ارسال الرساله اليه'
sendMessageText(SUDO,Text,msg.id_/2097152/0.5,'md')
return false
...
sendMessagePhoto(id_user, msg.id_, 0, 1, nil,msg.content_.photo_.sizes_[0].photo_.persistent_id_,(msg.content_.caption_ or ''))    
local Name = '['..string.sub(data.first_name_,0, 40)..'](tg://user?id='..id_user..')'
local Text = '🗣┋ المستخدم » '..Name..'\n☑┋ تم ارسال الرساله اليه'
sendMessageText(SUDO,Text,msg.id_/2097152/0.5,'md')
return false
...
sendMessageDocument(id_user, msg.id_, 0, 1,nil, msg.content_.animation_.animation_.persistent_id_)    
local Name = '['..string.sub(data.first_name_,0, 40)..'](tg://user?id='..id_user..')'
local Text = '🗣┋ المستخدم » '..Name..'\n☑┋ تم ارسال الرساله اليه'
sendMessageText(SUDO,Text,msg.id_/2097152/0.5,'md')
return false
...
sendMessageVoice(id_user, msg.id_, 0, 1, nil, msg.content_.voice_.voice_.persistent_id_)    
local Name = '['..string.sub(data.first_name_,0, 40)..'](tg://user?id='..id_user..')'
local Text = '🗣┋ المستخدم » '..Name..'\n☑┋ تم ارسال الرساله اليه'
sendMessageText(SUDO,Text,msg.id_/2097152/0.5,'md')
return false
...
sendMessage(msg.chat_id_, msg.id_,"☑┋ تم الغاء حفظ كليشه ستارت") 
redis:del('Globla:'..bot_id..'Start:Bots') 
return false
end
redis:set('Globla:'..bot_id.."Start:Bot",text)  
sendMessage(msg.chat_id_, msg.id_,'☑┋ تم حفظ كليشه ستارت') 
redis:del('Globla:'..bot_id..'Start:Bots') 
return false
end
if text == 'ضع كليشه ستارت 🔖' and SudoBot(msg) then 
redis:set('Globla:'..bot_id..'Start:Bots',true) 
sendMessage(msg.chat_id_, msg.id_,'📥┋ارسل لي الكليشه الان') 
return false
end
if text == 'حذف كليشه ستارت 🚫' and SudoBot(msg) then 
redis:del('Globla:'..bot_id..'Start:Bot') 
sendMessage(msg.chat_id_, msg.id_,'☑┋تم حذف كليشه ستارت') 
end
if text == "تغير رساله الاشتراك 📧" and SudoBot(msg) then  
redis:setex('Globla:'..bot_id.."textch:user" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_, 360, true)  
sendMessage(msg.chat_id_, msg.id_, '📃┋ حسنآ ارسل لي النص الذي تريده') 
return false  
end
if text == "حذف رساله الاشتراك 📃" and SudoBot(msg) then  
redis:del('Globla:'..bot_id..'text:ch:user')
sendMessage(msg.chat_id_, msg.id_, "🗑┋ تم مسح رساله الاشتراك ") 
return false  
end
----------------------------------------------------------------------------
if text == 'تفعيل التواصل 🔛' and SudoBot(msg) then  
sendMessage(msg.chat_id_, msg.id_, '☑┋ تم تفعيل التواصل')
redis:del('Globla:'..bot_id..'Twal:Bot'..msg.chat_id_)
end
if text == 'تعطيل التواصل ✖' and SudoBot(msg) then  
sendMessage(msg.chat_id_, msg.id_, '🚫┋ تم تعطيل التواصل')
redis:set('Globla:'..bot_id..'Twal:Bot'..msg.chat_id_, true)
end
----------------------------------------------------------------------------
if text == 'تفعيل الوضع الخدمي 🔓' and SudoBot(msg) then  
sendMessage(msg.chat_id_, msg.id_, '☑┋ تم تفعيل الوضع الخدمي')
redis:del('Globla:'..bot_id..'lock:Free'..msg.chat_id_)
end
if text == 'تعطيل الوضع الخدمي 🔏' and SudoBot(msg) then  
sendMessage(msg.chat_id_, msg.id_, '🚫┋ تم تعطيل الوضع الخدمي')
...
sendMessage(msg.chat_id_, msg.id_,'♻┋ تم تحديث السورس') 
dofile('GLOBLA.lua')  
end
if text == "ضع اسم للبوت ©" and SudoBot(msg) then  
redis:setex('Globla:'..bot_id..'Set:Name:Bot'..msg.sendMessageer_user_id_,300,true) 
sendMessage(msg.chat_id_, msg.id_,"☑┋ ارسل لي الاسم الان ")  
return false
end
if text == 'الاحصائيات 📈' and SudoBot(msg) then 
local Groups = redis:scard('Globla:'..bot_id..'Chek:Groups')  
local Users = redis:scard('Globla:'..bot_id..'UsersBot')  
Text = '📈┋ الاحصائيات : \n'..'👥┋ عدد المجموعات المفعله : {'..Groups..'}'..'\n👤┋ عدد المشتركين في البوت : {'..Users..'}'
sendMessage(msg.chat_id_, msg.id_,Text) 
return false
end
if text == ("المطورين 🕹") and SudoBot(msg) then
local list = redis:smembers('Globla:'..bot_id..'Sudo:User')
t = "\n📃┋ قائمة مطورين البوت \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
...
sendMessage(msg.chat_id_, msg.id_, t)
end
...
redis:setex('Globla:'..bot_id.."Send:Bc:Pv" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_, 600, true) 
sendMessage(msg.chat_id_, msg.id_,"📨┋ارسل لي سواء >> { ملصق, متحركه, صوره, رساله }\n☑┋ للخروج ارسل الغاء ") 
return false
end 
if text=="اذاعه 📌" and msg.reply_to_message_id_ == 0 and SudoBot(msg) then 
redis:setex('Globla:'..bot_id.."Send:Bc:Grops" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_, 600, true) 
sendMessage(msg.chat_id_, msg.id_,"📨┋ارسل لي سواء >> { ملصق, متحركه, صوره, رساله }\n☑┋ للخروج ارسل الغاء ") 
return false
end  
if text=="اذاعه بالتوجيه 🔖" and msg.reply_to_message_id_ == 0  and SudoBot(msg) then 
redis:setex('Globla:'..bot_id.."Send:Fwd:Grops" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_, 600, true) 
sendMessage(msg.chat_id_, msg.id_,"📥┋ ارسل لي التوجيه الان") 
return false
end 
if text=="اذاعه بالتوجيه خاص 👤" and msg.reply_to_message_id_ == 0  and SudoBot(msg) then 
redis:setex('Globla:'..bot_id.."Send:Fwd:Pv" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_, 600, true) 
sendMessage(msg.chat_id_, msg.id_,"📥┋ ارسل لي التوجيه الان") 
return false
end 
...
local pv = redis:smembers('Globla:'..bot_id..'UsersBot')  
local sendMessageok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]},function(arg,dataq)
tdcli_function ({ ID = "SendChatAction",chat_id_ = pv[i], action_ = {  ID = "SendMessageTypingAction", progress_ = 100} },function(arg,data) 
if data.ID and data.ID == "Ok"  then
else
redis:srem('Globla:'..bot_id..'UsersBot',pv[i])  
sendMessageok = sendMessageok + 1
end
if #pv == i then 
if sendMessageok == 0 then
sendMessage(msg.chat_id_, msg.id_,'✖┋لا يوجد مشتركين وهميين')   
else
local ok = #pv - sendMessageok
sendMessage(msg.chat_id_, msg.id_,'👥┋عدد المشتركين الان ←{ '..#pv..' }\n🚸┋تم العثور على ←{ '..sendMessageok..' } مشترك قام بحظر البوت\n👥┋اصبح عدد المشتركين الان ←{ '..ok..' } مشترك')   
end
end
end,nil)
end,nil)
...
local group = redis:smembers('Globla:'..bot_id..'Chek:Groups')  
local w = 0
local q = 0
for i = 1, #group do
...
redis:srem('Globla:'..bot_id..'Chek:Groups',group[i])  
w = w + 1
end
...
if #group == i then 
if (w + q) == 0 then
sendMessage(msg.chat_id_, msg.id_,'👥┋لا توجد مجموعات وهميه ')   
else
local zain = (w + q)
local sendMessageok = #group - zain
if q == 0 then
zain = ''
else
...
sendMessage(msg.chat_id_, msg.id_,'👥┋ عدد المجموعات الان ← { '..#group..' } مجموعه '..globla..''..zain..'\n👥┋اصبح عدد المجموعات الان ← { '..sendMessageok..' } مجموعات\n')   
end
end
end,nil)
end
...
redis:setex('Globla:'..bot_id.."add:ch:jm" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_, 360, true)  
sendMessage(msg.chat_id_, msg.id_, '🔅┋ حسنآ ارسل لي معرف القناة') 
return false  
end
if text == "تفعيل الاشتراك الاجباري 🔭" and SudoBot(msg) then  
if redis:get('Globla:'..bot_id..'add:ch:id') then
local addchusername = redis:get('Globla:'..bot_id..'add:ch:username')
sendMessage(msg.chat_id_, msg.id_,"🔅┋ الاشتراك الاجباري مفعل \n📡┋قناة الاشتراك » ["..addchusername.."]") 
else
redis:setex('Globla:'..bot_id.."add:ch:jm" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_, 360, true)  
sendMessage(msg.chat_id_, msg.id_,"™️┋ لم يتم وضع القناة \n📡┋لتعين قناة الاشتراك \n🔅┋ارسل معرف القناة") 
end
return false  
end
if text == "تعطيل الاشتراك الاجباري 🚫" and SudoBot(msg) then  
redis:del('Globla:'..bot_id..'add:ch:id')
redis:del('Globla:'..bot_id..'add:ch:username')
sendMessage(msg.chat_id_, msg.id_, "🚫┋ تم تعطيل الاشتراك الاجباري ") 
return false  
end
if text == "الاشتراك الاجباري 📡" and SudoBot(msg) then  
if redis:get('Globla:'..bot_id..'add:ch:username') then
local addchusername = redis:get('Globla:'..bot_id..'add:ch:username')
sendMessage(msg.chat_id_, msg.id_, "☑┋ تم تفعيل الاشتراك الاجباري \n📡┋قناة الاشتراك » ["..addchusername.."]") 
else
sendMessage(msg.chat_id_, msg.id_, "✖┋ لا يوجد قناة في الاشتراك الاجباري ") 
end
return false  
end
----------------------------------------------------------------------------
...
sendMessage(msg.chat_id_,msg.id_,"✖┋ عذرا عزيزي المستخدم هذا معرف قناة يرجى استخدام الامر بصوره صحيحه !")   
return false 
end      
redis:sadd('Globla:'..bot_id..'Sudo:User', result.id_)
...
sendMessage(msg.chat_id_, msg.id_, texts)
end
...
redis:sadd('Globla:'..bot_id..'Sudo:User', userid)
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم ترقيته مطور في البوت'
sendMessage(msg.chat_id_, msg.id_, usertext..status)
else
usertext = '\n👤┋المستخدم » '..userid..''
status  = '\n🔅┋ تم ترقيته مطور في البوت'
sendMessage(msg.chat_id_, msg.id_, usertext..status)
...
function start_function(extra, result, success)
if result.id_ then
redis:srem('Globla:'..bot_id..'Sudo:User', result.id_)
usertext = '\n👤┋المستخدم » ['..result.title_..'](t.me/'..(username or 'GLOBLA')..')'
...
redis:srem('Globla:'..bot_id..'Sudo:User', userid)
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم تنزيله من المطورين'
...
--------------------------------------------------------------------------------------------------------------
if redis:get('Globla:'..bot_id..'Set:Name:Bot'..msg.sendMessageer_user_id_) then 
if text == 'الغاء' or text == 'الغاء ✖' then   
sendMessage(msg.chat_id_, msg.id_,"☑┋ تم الغاء حفظ اسم البوت") 
redis:del('Globla:'..bot_id..'Set:Name:Bot'..msg.sendMessageer_user_id_) 
return false  
end 
redis:del('Globla:'..bot_id..'Set:Name:Bot'..msg.sendMessageer_user_id_) 
redis:set('Globla:'..bot_id..'Name:Bot',text) 
sendMessage(msg.chat_id_, msg.id_, "📥┋ تم حفظ اسم البوت")  
return false
end 
if text and redis:get('Globla:'..bot_id.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sendMessageer_user_id_) == "true" then
redis:set('Globla:'..bot_id.."Set:Cmd:Group:New"..msg.chat_id_,text)
sendMessage(msg.chat_id_, msg.id_,"🔰┋ارسل الامر الجديد")  
redis:del('Globla:'..bot_id.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sendMessageer_user_id_)
redis:set('Globla:'..bot_id.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sendMessageer_user_id_,"true1") 
return false
end
if text and redis:get('Globla:'..bot_id.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sendMessageer_user_id_) == "true1" then
local NewCmd = redis:get('Globla:'..bot_id.."Set:Cmd:Group:New"..msg.chat_id_)
redis:set('Globla:'..bot_id.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text,NewCmd)
redis:sadd('Globla:'..bot_id.."List:Cmd:Group:New"..msg.chat_id_,text)
sendMessage(msg.chat_id_, msg.id_,"🔰┋تم حفظ الامر")  
redis:del('Globla:'..bot_id.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sendMessageer_user_id_)
return false
end
if redis:get('Globla:'..bot_id.."Send:Bc:Pv" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_) then 
if text == 'الغاء' or text == 'الغاء ✖' then   
sendMessage(msg.chat_id_, msg.id_,"☑┋ تم الغاء الاذاعه للخاص") 
redis:del('Globla:'..bot_id.."Send:Bc:Pv" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_) 
return false
end 
local list = redis:smembers('Globla:'..bot_id..'UsersBot') 
if msg.content_.text_ then
for k,v in pairs(list) do 
sendMessage(v, 0,'['..msg.content_.text_..']')  
end
...
for k,v in pairs(list) do 
sendMessagePhoto(v, 0, 0, 1, nil, photo,(msg.content_.caption_ or ''))
end 
elseif msg.content_.animation_ then
for k,v in pairs(list) do 
sendMessageDocument(v, 0, 0, 1,nil, msg.content_.animation_.animation_.persistent_id_,(msg.content_.caption_ or ''))    
end 
elseif msg.content_.sticker_ then
for k,v in pairs(list) do 
sendMessageSticker(v, 0, 0, 1, nil, msg.content_.sticker_.sticker_.persistent_id_)   
end 
end
sendMessage(msg.chat_id_, msg.id_,"📣┋ تمت الاذاعه الى >>{"..#list.."} مشترك في البوت ")     
redis:del('Globla:'..bot_id.."Send:Bc:Pv" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_) 
end

if redis:get('Globla:'..bot_id.."Send:Bc:Grops" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_) then 
if text == 'الغاء' or text == 'الغاء ✖' then   
sendMessage(msg.chat_id_, msg.id_,"☑┋ تم الغاء الاذاعه") 
redis:del('Globla:'..bot_id.."Send:Bc:Grops" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_) 
return false
end 
local list = redis:smembers('Globla:'..bot_id..'Chek:Groups') 
if msg.content_.text_ then
...
sendMessage(msg.chat_id_, msg.id_,"📡┋ تمت الاذاعه الى >>{"..#list.."} مجموعه في البوت ")     
redis:del('Globla:'..bot_id.."Send:Bc:Grops" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_) 
end

if redis:get('Globla:'..bot_id.."Send:Fwd:Grops" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_) then 
if text == 'الغاء' or text == 'الغاء ✖' then   
sendMessage(msg.chat_id_, msg.id_,"☑┋ تم الغاء الاذاعه") 
redis:del('Globla:'..bot_id.."Send:Fwd:Grops" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_) 
return false  
end 
...
sendMessage(msg.chat_id_, msg.id_,"📡┋ تمت الاذاعه الى >>{"..#list.."} مجموعات في البوت ")     
redis:del('Globla:'..bot_id.."Send:Fwd:Grops" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_) 
end 
end
if redis:get('Globla:'..bot_id.."Send:Fwd:Pv" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_) then 
if text == 'الغاء' or text == 'الغاء ✖' then   
sendMessage(msg.chat_id_, msg.id_,"☑┋ تم الغاء الاذاعه") 
redis:del('Globla:'..bot_id.."Send:Fwd:Pv" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_) 
return false  
end 
if msg.forward_info_ then 
local list = redis:smembers('Globla:'..bot_id..'UsersBot')   
...
if redis:get('Globla:'..bot_id.."add:ch:jm" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_) then 
if text and text:match("^الغاء$") then 
sendMessage(msg.chat_id_, msg.id_, "✖┋ تم الغاء الامر ") 
redis:del('Globla:'..bot_id.."add:ch:jm" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)  
return false  end 
redis:del('Globla:'..bot_id.."add:ch:jm" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)  
...
sendMessage(msg.chat_id_, msg.id_, '📮┋ المعرف لا يوجد فيه قناة')
return false  end
if data and data.type_ and data.type_.ID and data.type_.ID == 'PrivateChatconfig' then
sendMessage(msg.chat_id_, msg.id_, '📮┋ عذا لا يمكنك وضع معرف حسابات في الاشتراك ') 
return false  end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.is_supergroup_ == true then
sendMessage(msg.chat_id_, msg.id_,'📮┋ عذا لا يمكنك وضع معرف مجوعه في الاشتراك ') 
return false  end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.is_supergroup_ == false then
if data and data.type_ and data.type_.channel_ and data.type_.channel_.ID and data.type_.channel_.status_.ID == 'ChatMemberStatusEditor' then
sendMessage(msg.chat_id_, msg.id_,'🔅┋ البوت ادمن في القناة \n☑┋ تم تفعيل الاشتراك الاجباري في \n🎟┋ ايدي القناة ('..data.id_..')\n📡┋ معرف القناة ([@'..data.type_.channel_.username_..'])') 
redis:set('Globla:'..bot_id..'add:ch:id',data.id_)
redis:set('Globla:'..bot_id..'add:ch:username','@'..data.type_.channel_.username_)
else
sendMessage(msg.chat_id_, msg.id_,'⚠️┋ البوت ليس ادمن في القناة يرجى ترقيته ادمن ثم اعادة المحاوله ') 
end
return false  
end
end,nil)
end
if redis:get('Globla:'..bot_id.."textch:user" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_) then 
if text and text:match("^الغاء$") then 
sendMessage(msg.chat_id_, msg.id_, "✖┋ تم الغاء الامر ") 
redis:del('Globla:'..bot_id.."textch:user" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)  
return false  end 
redis:del('Globla:'..bot_id.."textch:user" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)  
local texxt = string.match(text, "(.*)") 
redis:set('Globla:'..bot_id..'text:ch:user',texxt)
sendMessage(msg.chat_id_, msg.id_,'☑┋ تم تغيير رسالة الاشتراك بنجاح ')
end
...
tdcli_function({ID = "GetUser",user_id_=msg.sendMessageer_user_id_},function(extra,result) 
...
if redis:get('Globla:'..bot_id..'Change:Chat:Photo'..msg.chat_id_..':'..msg.sendMessageer_user_id_) then 
...
sendMessage(msg.chat_id_, msg.id_,'✖┋ عذرا البوت ليس ادمن يرجى ترقيتي والمحاوله لاحقا') 
redis:del('Globla:'..bot_id..'Change:Chat:Photo'..msg.chat_id_..':'..msg.sendMessageer_user_id_) 
return false  end
if data.message_ == "CHAT_ADMIN_REQUIRED" then 
sendMessage(msg.chat_id_, msg.id_,'✖┋ ليس لدي صلاحية تغيير معلومات المجموعه يرجى المحاوله لاحقا') 
redis:del('Globla:'..bot_id..'Change:Chat:Photo'..msg.chat_id_..':'..msg.sendMessageer_user_id_) 
else
sendMessage(msg.chat_id_, msg.id_,'☑┋ تم تغيير صورة المجموعه') 
end
end, nil) 
redis:del('Globla:'..bot_id..'Change:Chat:Photo'..msg.chat_id_..':'..msg.sendMessageer_user_id_) 
end   
...
--------------------------------------------------------------------------------------------------------------
if redis:get('Globla:'..bot_id.."Set:Description" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_) then  
if text == 'الغاء' then 
sendMessage(msg.chat_id_, msg.id_,"✖┋ تم الغاء وضع الوصف") 
redis:del('Globla:'..bot_id.."Set:Description" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)
return false  
end 
redis:del('Globla:'..bot_id.."Set:Description" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)   
https.request('https://api.telegram.org/bot'..token..'/setChatDescription?chat_id='..msg.chat_id_..'&description='..text) 
sendMessage(msg.chat_id_, msg.id_,'☑┋ تم تغيير وصف المجموعه')   
return false  
end 
--------------------------------------------------------------------------------------------------------------
if redis:get('Globla:'..bot_id.."Welcome:Group" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_) then 
if text == 'الغاء' then 
sendMessage(msg.chat_id_, msg.id_,"✖┋ تم الغاء حفظ الترحيب") 
redis:del('Globla:'..bot_id.."Welcome:Group" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)  
return false  
end 
redis:del('Globla:'..bot_id.."Welcome:Group" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)  
redis:set('Globla:'..bot_id..'Get:Welcome:Group'..msg.chat_id_,text) 
sendMessage(msg.chat_id_, msg.id_,'☑┋ تم حفظ ترحيب المجموعه')   
return false   
end
--------------------------------------------------------------------------------------------------------------
if redis:get('Globla:'..bot_id.."Set:Priovate:Group:Link"..msg.chat_id_..""..msg.sendMessageer_user_id_) then
if text == 'الغاء' then
sendMessage(msg.chat_id_,msg.id_,"📥┋ تم الغاء حفظ الرابط")       
redis:del('Globla:'..bot_id.."Set:Priovate:Group:Link"..msg.chat_id_..""..msg.sendMessageer_user_id_) 
return false
end
if text and text:match("(https://telegram.me/joinchat/%S+)") or text and text:match("(https://t.me/joinchat/%S+)") then     
local Link = text:match("(https://telegram.me/joinchat/%S+)") or text:match("(https://t.me/joinchat/%S+)")   
redis:set('Globla:'..bot_id.."Private:Group:Link"..msg.chat_id_,Link)
sendMessage(msg.chat_id_,msg.id_,"📥┋ تم حفظ الرابط بنجاح")       
redis:del('Globla:'..bot_id.."Set:Priovate:Group:Link"..msg.chat_id_..""..msg.sendMessageer_user_id_) 
return false 
end
end 
...
sendMessage(msg.chat_id_,0,"👤┋المستخدم : {["..data.first_name_.."](T.ME/"..data.username_..")}\n📛┋["..GLOBLA_Msg.."] \n") 
else
sendMessage(msg.chat_id_,0,"👤┋المستخدم : {["..data.first_name_.."](T.ME/GLOBLA)}\n📛┋["..GLOBLA_Msg.."] \n") 
end
end,nil)   
DeleteMessage(msg.chat_id_, {[0] = msg.id_})     
return false
...
local post_count = tonumber(redis:get('Globla:'..bot_id..'floodc:'..msg.sendMessageer_user_id_..':'..msg.chat_id_) or 0)
...
redis:setex('Globla:'..bot_id..'floodc:'..msg.sendMessageer_user_id_..':'..msg.chat_id_, tonumber(redis:hget('Globla:'..bot_id.."flooding:settings:"..msg.chat_id_,"floodtime") or 3), post_count+1) 
...
redis:incr('Globla:'..bot_id..'Add:Contact'..msg.chat_id_..':'..msg.sendMessageer_user_id_) 
end
...
ked(msg.chat_id_,msg.sendMessageer_user_id_)
DeleteMessage(msg.chat_id_,{[0] = msg.id_}) 
...
RestrictChat(msg.chat_id_,msg.sendMessageer_user_id_)
DeleteMessage(msg.chat_id_,{[0] = msg.id_}) 
...
tdcli_function({ID = "GetUser",user_id_ = msg.sendMessageer_user_id_},function(arg,data) 
if data.username_ ~= false then
sendMessage(msg.chat_id_,0, "👤┋المستخدم » {[@"..data.username_.."]}\n📛┋ عذراً لايمكنك ارسل الملصق في المجموعه \n" ) 
else
sendMessage(msg.chat_id_,0, "👤┋المستخدم » {["..data.first_name_.."](T.ME/GLOBLA)}\n📛┋ عذراً لايمكنك ارسل الملصق في المجموعه \n" ) 
end
end,nil)   
...
sendMessage(msg.chat_id_, msg.id_,'🚸┋البوت ليس ادمن يرجى ترقيتي !') 
return false  
end
...
sendMessage(msg.chat_id_, msg.id_,'👥┋عدد اعضاء المجموعه اقل من *~ {'..(redis:get('Globla:'..bot_id..'Num:Add:Bot') or 0)..'* عضو')
return false
end
tdcli_function ({ID = "GetUser",user_id_ = msg.sendMessageer_user_id_},function(extra,result,success)
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,chat)  
if redis:sismember('Globla:'..bot_id..'Chek:Groups',msg.chat_id_) then
sendMessage(msg.chat_id_, msg.id_,'🔅┋المجموعه مفعله بالتأكيد ')
else
sendMessageText(msg.chat_id_,'\n👤┋ بواسطة : ['..string.sub(result.first_name_,0, 70)..'](tg://user?id='..result.id_..')\n☑┋ تم تفعيل المجموعه {'..chat.title_..'}',msg.id_/2097152/0.5,'md')
redis:sadd('Globla:'..bot_id..'Chek:Groups',msg.chat_id_)
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sendMessageer_user_id_},function(extra,result,success)
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,chat)  
if not redis:sismember('Globla:'..bot_id..'Chek:Groups',msg.chat_id_) then
sendMessage(msg.chat_id_, msg.id_,'☑┋المجموعه معطله سابقا ')
else
sendMessageText(msg.chat_id_,'\n👤┋ بواسطة : ['..string.sub(result.first_name_,0, 70)..'](tg://user?id='..result.id_..')\n🚫┋ تم تعطيل المجموعه {'..chat.title_..'}',msg.id_/2097152/0.5,'md')
redis:srem('Globla:'..bot_id..'Chek:Groups',msg.chat_id_)  
local Name = '['..result.first_name_..'](tg://user?id='..result.id_..')'
local NameChat = chat.title_
NameChat = NameChat:gsub('"',"") 
NameChat = NameChat:gsub('"',"") 
...
if text == 'تفعيل' and not Sudo(msg) and not redis:get('Globla:'..bot_id..'Free:Add:Bots') then 
if msg.can_be_deleted_ == false then 
sendMessage(msg.chat_id_, msg.id_,'🚸┋البوت ليس ادمن يرجى ترقيتي !') 
return false  
end
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sendMessageer_user_id_},function(extra,result,success)
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,chat)  
tdcli_function ({ID = "GetChatMember",chat_id_ = msg.chat_id_,user_id_ = msg.sendMessageer_user_id_},function(arg,da) 
if da and da.status_.ID == "ChatMemberStatusEditor" or da and da.status_.ID == "ChatMemberStatusCreator" then
if da and da.user_id_ == msg.sendMessageer_user_id_ then
...
if redis:sismember('Globla:'..bot_id..'Chek:Groups',msg.chat_id_) then
sendMessage(msg.chat_id_, msg.id_,'📮┋المجموعه مفعله سابقا ')
else
if tonumber(data.member_count_) < tonumber(redis:get('Globla:'..bot_id..'Num:Add:Bot') or 0) and not SudoBot(msg) then
sendMessage(msg.chat_id_, msg.id_,'👥┋عدد اعضاء المجموعه اقل من *~ {'..(redis:get('Globla:'..bot_id..'Num:Add:Bot') or 0)..'* عضو')
return false
...
sendMessageText(msg.chat_id_,'\n👤┋ بواسطة : ['..string.sub(result.first_name_,0, 70)..'](tg://user?id='..result.id_..')\n☑┋ تم تفعيل المجموعه {'..chat.title_..'}',msg.id_/2097152/0.5,'md')
redis:sadd('Globla:'..bot_id..'Chek:Groups',msg.chat_id_)  
redis:sadd('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_, msg.sendMessageer_user_id_)
local Name = '['..result.first_name_..'](tg://user?id='..result.id_..')'
local NumMember = data.member_count_
local NameChat = chat.title_
NameChat = NameChat:gsub('"',"") 
...
sendMessage(msg.chat_id_, msg.id_,'📌┋ تم تعيين عدد الاعضاء سيتم تفعيل المجموعات التي اعضائها اكثر من  >> {'..Num..'} عضو')
end
if text == 'تحديث السورس' and SudoBot(msg) then 
if AddChannel(msg.sendMessageer_user_id_) == false then
local textchuser = redis:get('Globla:'..bot_id..'text:ch:user')
...
os.execute('rm -rf GLOBLA.lua')
os.execute('wget https://raw.githubusercontent.com/DevGlobla/GLOBLA/master/GLOBLA.lua')
sendMessage(msg.chat_id_, msg.id_,'♻┋ تم تحديث السورس \n👤┋ مــن قـبـل ⊰• ['..Rutba(msg.sendMessageer_user_id_,msg.chat_id_)..'](T.ME/'..(data.username_ or 'GLOBLA')..')') 
dofile('GLOBLA.lua')  
end
...
local list = redis:smembers('Globla:'..bot_id.."List:Cmd:Group:New"..msg.chat_id_.."")
t = "🔰┋قائمه الاوامر المضافه  \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ \n"
for k,v in pairs(list) do
Cmds = redis:get('Globla:'..bot_id.."Set:Cmd:Group:New1"..msg.chat_id_..":"..v)
if Cmds then 
t = t..""..k.."- ("..v..") ~ {"..Cmds.."}\n"
else
t = t..""..k.."- ("..v..") \n"
...
local list = redis:smembers('Globla:'..bot_id.."List:Cmd:Group:New"..msg.chat_id_)
for k,v in pairs(list) do
redis:del('Globla:'..bot_id.."Set:Cmd:Group:New1"..msg.chat_id_..":"..v)
redis:del('Globla:'..bot_id.."List:Cmd:Group:New"..msg.chat_id_)
end
sendMessage(msg.chat_id_, msg.id_,"🔰┋تم مسح جميع الاوامر التي تم اضافتها")  
end
end
if text == "اضف امر" and Constructor(msg) then
redis:set('Globla:'..bot_id.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sendMessageer_user_id_,"true") 
sendMessage(msg.chat_id_, msg.id_,"🔰┋الان ارسل لي الامر القديم ..")  
return false
end
if text == "حذف امر" or text == "مسح امر" then 
if Constructor(msg) then
redis:set('Globla:'..bot_id.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sendMessageer_user_id_,"true") 
sendMessage(msg.chat_id_, msg.id_,"🔰┋ارسل الامر الذي قم بوضعه بدلا عن القديم")  
return false
end
end
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sendMessageer_user_id_},function(arg,data)  
sendMessage(msg.chat_id_, msg.id_,'👤┋بواسطه :  ['..Rutba(msg.sendMessageer_user_id_,msg.chat_id_)..'](T.ME/'..(data.username_ or 'GLOBLA')..') \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n🌐┋ تـم قفـل الدردشه ')  
end,nil)   
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sendMessageer_user_id_},function(arg,data) 
sendMessage(msg.chat_id_, msg.id_,'👤┋بواسطه :  ['..string.sub(data.first_name_,0,60)..'](T.ME/'..(data.username_ or 'GLOBLA')..')\n🎛┋ تـم قفـل التاك بالكتم')  
end,nil)   
...
function start_function(extra, result, success)
if result.sendMessageer_user_id_ == tonumber(SUDO) then
sendMessage(msg.chat_id_, msg.id_, "🚫┋ لا يمكنك حظر المطور الاساسي \n")
return false 
end
if tonumber(result.sendMessageer_user_id_) == tonumber(bot_id) then  
sendMessage(msg.chat_id_, msg.id_, "✖┋ لا تسطيع حظر البوت عام")
return false 
end
redis:sadd('Globla:'..bot_id..'GBan:User', result.sendMessageer_user_id_)
Group_Kick(result.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},
function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
...
function start_function(extra, result, success)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🚫┋ تم الغاء حظره عام من المجموعات'
sendMessage(msg.chat_id_, msg.id_, usertext..status)
end,nil)
redis:srem('Globla:'..bot_id..'GBan:User', result.sendMessageer_user_id_)
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, start_function, nil)
return false
end
...
redis:del('Globla:'..bot_id..'Sudo:User')
...
redis:sadd('Globla:'..bot_id..'Sudo:User', result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم ترقيته مطور في البوت'
sendMessage(msg.chat_id_, msg.id_, usertext..status)
...
function start_function(extra, result, success)
redis:srem('Globla:'..bot_id..'Sudo:User', result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم تنزيله من المطورين'
sendMessage(msg.chat_id_, msg.id_, usertext..status)
...
redis:del('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_)
sendMessage(msg.chat_id_, msg.id_, '\n🗑┋ تم مسح قائمه المنشئين الاساسين')
return false
end

if text == 'المنشئين الاساسين' and Sudo(msg) then
local list = redis:smembers('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_)
...
sendMessage(msg.chat_id_, msg.id_,"🗑┋ حساب المنشئ محذوف")
return false  
end
local UserName = (b.username_ or "Spirta")
sendMessage(msg.chat_id_, msg.id_,"👮‍♂️┋المالك : ["..b.first_name_.."](T.me/"..UserName..")")  
end,nil)   
end
end
end,nil)   
...
local UserName = (b.username_ or "Spirta")
sendMessage(msg.chat_id_, msg.id_,"👮‍♂️┋تم ترقية منشئ المجموعه : ["..b.first_name_.."](T.me/"..UserName..")")  
...
function start_function(extra, result, success)
redis:sadd('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
...
redis:sadd('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_, result.id_)
usertext = '\n👤┋المستخدم » ['..result.title_..'](t.me/'..(username or 'GLOBLA')..')'
status  = '\n🔅┋ تم ترقيته منشئ اساسي'
texts = usertext..status
else
...
redis:sadd('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_, userid)
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم ترقيته منشئ اساسي'
...
function start_function(extra, result, success)
redis:srem('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
...
function start_function(extra, result, success)
if result.id_ then
redis:srem('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_, result.id_)
usertext = '\n👤┋المستخدم » ['..result.title_..'](t.me/'..(username or 'GLOBLA')..')'
status  = '\n🔅┋ تم تنزيله من الاساسيين'
texts = usertext..status
else
...
redis:srem('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_, userid)
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم تنزيله من الاساسيين'
...
redis:del('Globla:'..bot_id..'Constructor'..msg.chat_id_)
texts = '🗑┋  تم مسح المنشئين '
sendMessage(msg.chat_id_, msg.id_, texts)
end

if text == ("المنشئين") and BasicConstructor(msg) then
local list = redis:smembers('Globla:'..bot_id..'Constructor'..msg.chat_id_)
t = "\n📃┋ قائمة المنشئين \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
local username = redis:get('Globla:'..bot_id.."user:Name" .. v)
if username then
...
function start_function(extra, result, success)
redis:sadd('Globla:'..bot_id..'Constructor'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم ترقيته منشئ'
sendMessage(msg.chat_id_, msg.id_, usertext..status)
...
redis:sadd('Globla:'..bot_id..'Constructor'..msg.chat_id_, result.id_)
usertext = '\n👤┋المستخدم » ['..result.title_..'](t.me/'..(username or 'GLOBLA')..')'
status  = '\n🔅┋ تم ترقيته منشئ'
texts = usertext..status
else
...
redis:sadd('Globla:'..bot_id..'Constructor'..msg.chat_id_, userid)
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم ترقيته منشئ'
...
function start_function(extra, result, success)
redis:srem('Globla:'..bot_id..'Constructor'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
...
function start_function(extra, result, success)
if result.id_ then
redis:srem('Globla:'..bot_id..'Constructor'..msg.chat_id_, result.id_)
usertext = '\n👤┋المستخدم » ['..result.title_..'](t.me/'..(username or 'GLOBLA')..')'
status  = '\n🔅┋ تم تنزيله من المنشئين'
texts = usertext..status
else
...
redis:srem('Globla:'..bot_id..'Constructor'..msg.chat_id_, userid)
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم تنزيله من المنشئين'
...
redis:del('Globla:'..bot_id..'Manager'..msg.chat_id_)
texts = '🗑┋  تم مسح المدراء '
sendMessage(msg.chat_id_, msg.id_, texts)
end
if text == ("المدراء") and Constructor(msg) then
local list = redis:smembers('Globla:'..bot_id..'Manager'..msg.chat_id_)
t = "\n📃┋ قائمة المدراء \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
local username = redis:get('Globla:'..bot_id.."user:Name" .. v)
if username then
...
function start_function(extra, result, success)
redis:sadd('Globla:'..bot_id..'Manager'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم ترقيته مدير'
sendMessage(msg.chat_id_, msg.id_, usertext..status)
...
redis:sadd('Globla:'..bot_id..'Manager'..msg.chat_id_, result.id_)
usertext = '\n👤┋المستخدم » ['..result.title_..'](t.me/'..(username or 'GLOBLA')..')'
status  = '\n🔅┋ تم ترقيته مدير'
texts = usertext..status
else
...
redis:sadd('Globla:'..bot_id..'Manager'..msg.chat_id_, userid)
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم ترقيته مدير'
...
function start_function(extra, result, success)
redis:srem('Globla:'..bot_id..'Manager'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
...
function start_function(extra, result, success)
if result.id_ then
redis:srem('Globla:'..bot_id..'Manager'..msg.chat_id_, result.id_)
usertext = '\n👤┋المستخدم » ['..result.title_..'](t.me/'..(username or 'GLOBLA')..')'
status  = '\n🔅┋ تم تنزيله من المدراء'
texts = usertext..status
else
...
redis:srem('Globla:'..bot_id..'Manager'..msg.chat_id_, userid)
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم تنزيله من المدراء'
...
redis:sadd('Globla:'..bot_id.."Mod:User"..msg.chat_id_, admins[i].user_id_)
num2 = num2 + 1
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,b) 
if b.username_ == true then
end
if b.first_name_ == false then
redis:srem('Globla:'..bot_id.."Mod:User"..msg.chat_id_, admins[i].user_id_)
end
end,nil)   
else
redis:srem('Globla:'..bot_id.."Mod:User"..msg.chat_id_, admins[i].user_id_)
...
if num2 == 0 then
sendMessage(msg.chat_id_, msg.id_,"📛┋ لا توجد ادمنية ليتم رفعهم") 
else
sendMessage(msg.chat_id_, msg.id_,"☑┋ تمت ترقية { "..num2.." } من ادمنية المجموعه") 
end
end,nil)   
end
if text == 'مسح الادمنيه' and Manager(msg) then
redis:del('Globla:'..bot_id..'Mod:User'..msg.chat_id_)
sendMessage(msg.chat_id_, msg.id_, '🗑┋  تم مسح  قائمة الادمنية  ')
end
if text == ("الادمنيه") then
local list = redis:smembers('Globla:'..bot_id..'Mod:User'..msg.chat_id_)
t = "\n📃┋ قائمة الادمنيه \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
local username = redis:get('Globla:'..bot_id.."user:Name" .. v)
if username then
...
sendMessage(msg.chat_id_, msg.id_,'☑┋ تم تعطيل الرفع من قبل المنشئين') 
return false
end
redis:sadd('Globla:'..bot_id..'Mod:User'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم ترقيته ادمن'
sendMessage(msg.chat_id_, msg.id_, usertext..status)
...
redis:sadd('Globla:'..bot_id..'Mod:User'..msg.chat_id_, result.id_)
usertext = '\n👤┋المستخدم » ['..result.title_..'](t.me/'..(username or 'GLOBLA')..')'
status  = '\n🔅┋ تم ترقيته ادمن'
texts = usertext..status
else
...
redis:sadd('Globla:'..bot_id..'Mod:User'..msg.chat_id_, userid)
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم ترقيته ادمن'
...
function start_function(extra, result, success)
redis:srem('Globla:'..bot_id..'Mod:User'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
...
function start_function(extra, result, success)
if result.id_ then
redis:srem('Globla:'..bot_id..'Mod:User'..msg.chat_id_, result.id_)
usertext = '\n👤┋المستخدم » ['..result.title_..'](t.me/'..(username or 'GLOBLA')..')'
status  = '\n🔅┋ تم تنزيله من الادمنيه'
texts = usertext..status
else
...
redis:srem('Globla:'..bot_id..'Mod:User'..msg.chat_id_, userid)
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم تنزيله من الادمنيه'
...
sendMessage(msg.chat_id_, msg.id_,'🚫┋ تم تعطيل الطرد من قبل المنشئين') 
return false
end
function start_function(extra, result, success)
if tonumber(result.sendMessageer_user_id_) == tonumber(bot_id) then  
sendMessage(msg.chat_id_, msg.id_, "⚠┋هذا بوت لا يمكنني الطرد ")
return false 
end
if Can_or_NotCan(result.sendMessageer_user_id_, msg.chat_id_) == true then
sendMessage(msg.chat_id_, msg.id_, '\n⚠┋ عذرا لا يمكنك طرد ( '..Rutba(result.sendMessageer_user_id_,msg.chat_id_)..' )')
else
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = msg.chat_id_, user_id_ = result.id_, status_ = { ID = "ChatMemberStatusKicked" },},function(arg,data) 
if (data and data.code_ and data.code_ == 400 and data.message_ == "CHAT_ADMIN_REQUIRED") then 
sendMessage(msg.chat_id_, msg.id_,'⚠┋ ليس لدي صلاحية حظر المستخدمين يرجى تفعيلها !') 
return false  
end
if msg.can_be_deleted_ == false then 
sendMessage(msg.chat_id_, msg.id_,'🚸┋ البوت ليس ادمن يرجى ترقيتي !') 
...
usertext = '\n👤┋ المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
statusk  = '\n🚷┋ تم طرده من هنا'
sendMessage(msg.chat_id_, msg.id_, usertext..statusk)
end,nil)
Group_Kick(result.chat_id_, result.sendMessageer_user_id_)
end,nil)   
end
...
sendMessage(msg.chat_id_,msg.id_,"⚠┋ عذرا عزيزي المستخدم هاذا معرف قناة يرجى استخدام الامر بصوره صحيحه !")   
return false 
end      
if (data and data.code_ and data.code_ == 400 and data.message_ == "CHAT_ADMIN_REQUIRED") then 
sendMessage(msg.chat_id_, msg.id_,'⚠┋ ليس لدي صلاحية حظر المستخدمين يرجى تفعيلها !') 
...
sendMessage(msg.chat_id_, msg.id_, '⚠┋ لا يوجد حساب بهاذا المعرف')
end
end
tdcli_function ({ID = "SearchPublicChat",username_ = username}, start_function, nil)
return false
...
redis:del('Globla:'..bot_id..'Special:User'..msg.chat_id_)
sendMessage(msg.chat_id_, msg.id_, '🗑┋  تم مسح  قائمة الاعضاء المميزين  ')
end
if text == ("المميزين") and Mod(msg) then
local list = redis:smembers('Globla:'..bot_id..'Special:User'..msg.chat_id_)
...
function start_function(extra, result, success)
redis:sadd('Globla:'..bot_id..'Special:User'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
local  statuss  = '\n🔅┋ تم ترقيته مميز هنا '
sendMessage(msg.chat_id_, msg.id_, usertext..statuss)
end,nil)
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, start_function, nil)
return false
...
redis:sadd('Globla:'..bot_id..'Special:User'..msg.chat_id_, result.id_)
usertext = '\n👤┋المستخدم » ['..result.title_..'](t.me/'..(username or 'GLOBLA')..')'
local  statuss  = '\n🔅┋ تم ترقيته مميز هنا'
...
redis:sadd('Globla:'..bot_id..'Special:User'..msg.chat_id_, userid)
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
local  statuss  = '\n🔅┋ تم ترقيته مميز هنا'
...
function start_function(extra, result, success)
redis:srem('Globla:'..bot_id..'Special:User'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
...
function start_function(extra, result, success)
if result.id_ then
redis:srem('Globla:'..bot_id..'Special:User'..msg.chat_id_, result.id_)
usertext = '\n👤┋المستخدم » ['..result.title_..'](t.me/'..(username or 'GLOBLA')..')'
status  = '\n🔅┋ تم تنزيله من المميزين'
texts = usertext..status
else
...
redis:srem('Globla:'..bot_id..'Special:User'..msg.chat_id_, userid)
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n??┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔅┋ تم تنزيله من المميزين'
...
if redis:get('Globla:'..bot_id..'Lock:kick'..msg.chat_id_) and not Constructor(msg) then
sendMessage(msg.chat_id_, msg.id_,'🚫┋ تم تعطيل الحظر من قبل المنشئين') 
return false
end
function start_function(extra, result, success)
if tonumber(result.sendMessageer_user_id_) == tonumber(bot_id) then  
sendMessage(msg.chat_id_, msg.id_, "⚠┋هذا بوت لا يمكنني الحظر ")
return false 
end
if Can_or_NotCan(result.sendMessageer_user_id_, msg.chat_id_) == true then
sendMessage(msg.chat_id_, msg.id_, '\n⚠┋ عذرا لا يمكنك حظر ( '..Rutba(result.sendMessageer_user_id_,msg.chat_id_)..' )')
else
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = msg.chat_id_, user_id_ = result.sendMessageer_user_id_, status_ = { ID = "ChatMemberStatusKicked" },},function(arg,data) 
if (data and data.code_ and data.code_ == 400 and data.message_ == "CHAT_ADMIN_REQUIRED") then 
sendMessage(msg.chat_id_, msg.id_,'⚠┋ ليس لدي صلاحية حظر المستخدمين يرجى تفعيلها !') 
return false  
end
...
redis:sadd('Globla:'..bot_id..'Ban:User'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋ المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🚫┋ تم حظره من المجموعه'
sendMessage(msg.chat_id_, msg.id_, usertext..status)
...
function start_function(extra, result, success)
if tonumber(result.sendMessageer_user_id_) == tonumber(bot_id) then
sendMessage(msg.chat_id_, msg.id_, '☑️┋ انا لست محظورا \n') 
return false 
end
redis:srem('Globla:'..bot_id..'Ban:User'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋ المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🚫┋ تم الغاء حظره من هنا'
sendMessage(msg.chat_id_, msg.id_, usertext..status)
end,nil)
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = msg.chat_id_, user_id_ = result.sendMessageer_user_id_, status_ = { ID = "ChatMemberStatusLeft" },},function(arg,ban) end,nil)   
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, start_function, nil)
return false
end
...
sendMessage(msg.chat_id_, msg.id_, '🗑┋  تم مسح قائمه المكتومين ')
end
...
function start_function(extra, result, success)
if tonumber(result.sendMessageer_user_id_) == tonumber(bot_id) then  
sendMessage(msg.chat_id_, msg.id_, "⚠┋هذا بوت لا يمكنني الكتم ")
return false 
end
if Can_or_NotCan(result.sendMessageer_user_id_, msg.chat_id_) == true then
sendMessage(msg.chat_id_, msg.id_, '\n⚠┋ عذرا لا يمكنك كتم ( '..Rutba(result.sendMessageer_user_id_,msg.chat_id_)..' )')
else
if msg.can_be_deleted_ == false then 
sendMessage(msg.chat_id_, msg.id_,'🚸┋ البوت ليس ادمن يرجى ترقيتي !') 
return false  
end
redis:sadd('Globla:'..bot_id..'Muted:User'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋ المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🔇┋ تم كتمه من هنا'
sendMessage(msg.chat_id_, msg.id_, usertext..status)
...
function start_function(extra, result, success)
redis:srem('Globla:'..bot_id..'Muted:User'..msg.chat_id_, result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋ المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
...
function start_function(extra, result, success)
if Can_or_NotCan(result.sendMessageer_user_id_, msg.chat_id_) then
sendMessage(msg.chat_id_, msg.id_, '\n⚠┋ عذرا لا يمكنك طرد او حظر او كتم او تقيد ( '..Rutba(result.sendMessageer_user_id_,msg.chat_id_)..' )')
else
https.request("https://api.telegram.org/bot"..token.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id="..result.sendMessageer_user_id_)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋ المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
...
function start_function(extra, result, success)
https.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. result.sendMessageer_user_id_ .. "&can_sendMessage_messages=True&can_sendMessage_media_messages=True&can_sendMessage_other_messages=True&can_add_web_page_previews=True")
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(arg,data) 
usertext = '\n👤┋ المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🚸┋ تم الغاء تقيده'
sendMessage(msg.chat_id_, msg.id_, usertext..status)
...
function start_function(extra, result, success)
if result.id_ then
https.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. result.id_ .. "&can_sendMessage_messages=True&can_sendMessage_media_messages=True&can_sendMessage_other_messages=True&can_add_web_page_previews=True")
usertext = '\n👤┋ المستخدم » ['..result.title_..'](t.me/'..(username or 'GLOBLA')..')'
status  = '\n🚸┋ تم الغاء تقيده'
texts = usertext..status
else
...
https.request("https://api.telegram.org/bot" .. token .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..userid.. "&can_sendMessage_messages=True&can_sendMessage_media_messages=True&can_sendMessage_other_messages=True&can_add_web_page_previews=True")
tdcli_function ({ID = "GetUser",user_id_ = userid},function(arg,data) 
if data.first_name_ then
usertext = '\n👤┋ المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'
status  = '\n🚸┋ تم الغاء تقيده'
...
sendMessage(msg.chat_id_, msg.id_,Textt)
else
Text = '✖┋ المعرف خطأ'
sendMessage(msg.chat_id_, msg.id_,Text)
end
...
function start_function(extra, result, success)
if redis:sismember('Globla:'..bot_id..'Muted:User'..msg.chat_id_,result.sendMessageer_user_id_) then
Muted = 'مكتوم'
else
Muted = 'غير مكتوم'
end
if redis:sismember('Globla:'..bot_id..'Ban:User'..msg.chat_id_,result.sendMessageer_user_id_) then
Ban = 'محظور'
else
Ban = 'غير محظور'
end
if redis:sismember('Globla:'..bot_id..'GBan:User',result.sendMessageer_user_id_) then
GBan = 'محظور عام'
else
GBan = 'غير محظور عام'
end
...
sendMessage(msg.chat_id_, msg.id_, usertext..status)
https.request("https://api.telegram.org/bot"..token.."/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.sendMessageer_user_id_.."&can_change_info=false&can_delete_messages=false&can_invite_users=True&can_restrict_members=false&can_pin_messages=True&can_promote_members=false")
end,nil)
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, start_function, nil)
return false
...
sendMessage(msg.chat_id_, msg.id_, '✖┋ لا يوجد حساب بهذا المعرف')
end
end
tdcli_function ({ID = "SearchPublicChat",username_ = username}, start_function, nil)
return false
...
sendMessage(msg.chat_id_, msg.id_, usertext..status)
https.request("https://api.telegram.org/bot"..token.."/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.sendMessageer_user_id_.."&can_change_info=false&can_delete_messages=false&can_invite_users=false&can_restrict_members=false&can_pin_messages=false&can_promote_members=false")
end,nil)
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, start_function, nil)
return false
...
sendMessage(msg.chat_id_, msg.id_, usertext..status)
https.request("https://api.telegram.org/bot"..token.."/promoteChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" ..result.sendMessageer_user_id_.."&can_change_info=True&can_delete_messages=True&can_invite_users=True&can_restrict_members=True&can_pin_messages=True&can_promote_members=True")
end,nil)
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, start_function, nil)
return false
...
sendMessage(msg.chat_id_,msg.id_,"👨‍✈️┋ التثبيت والغاء التثبيت تم قفله من قبل المنشئين")  
return false  
end
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.reply_to_message_id_,disable_notification_ = 1},function(arg,data) 
if data.ID == "Ok" then
sendMessage(msg.chat_id_, msg.id_,"📌┋ تم تثبيت الرساله")   
redis:set('Globla:'..bot_id..'Pin:Id:Msg'..msg.chat_id_,msg.reply_to_message_id_)
elseif data.code_ == 6 then
sendMessage(msg.chat_id_,msg.id_,"✖┋ انا لست ادمن هنا يرجى ترقيتي ادمن ثم اعد المحاوله")  
elseif data.message_ == "CHAT_NOT_MODIFIED" then
sendMessage(msg.chat_id_,msg.id_,"📌┋ لا توجد رساله مثبته")  
elseif data.message_ == "CHAT_ADMIN_REQUIRED" then
sendMessage(msg.chat_id_,msg.id_,"☑┋ ليست لدي صلاحية التثبيت يرجى التحقق من الصلاحيات")  
end
end,nil) 
end
...
if data.ID == "Ok" then
sendMessage(msg.chat_id_, msg.id_,"📌┋ تم الغاء تثبيت الرساله")   
...
sendMessage(msg.chat_id_, msg.id_,'🌀┋ تم وضع عدد التكرار ('..Num..')')  
end 
...
sendMessage(msg.chat_id_, msg.id_,'🌀┋ تم وضع زمن التكرار ('..Num..')') 
end
if text == "ضع رابط" or text == 'وضع رابط' then
if msg.reply_to_message_id_ == 0  and Mod(msg) then  
sendMessage(msg.chat_id_,msg.id_,"📎┋ ارسل رابط المجموعه او رابط قناة المجموعه")
redis:setex('Globla:'..bot_id.."Set:Priovate:Group:Link"..msg.chat_id_..""..msg.sendMessageer_user_id_,120,true) 
return false
end
end
...
sendMessage(msg.chat_id_, msg.id_,"☑┋تم تفعيل الرابط") 
return false  
end
end
...
sendMessage(msg.chat_id_, msg.id_,linkgp)              
end            
end
if text == 'مسح الرابط' or text == 'حذف الرابط' then
if Mod(msg) then
sendMessage(msg.chat_id_,msg.id_,"🗑┋ تم مسح الرابط ")           
...
redis:set('Globla:'..bot_id..'Change:Chat:Photo'..msg.chat_id_..':'..msg.sendMessageer_user_id_,true) 
sendMessage(msg.chat_id_, msg.id_,'🎇┋ ارسل لي الصوره') 
return false
end
...
sendMessage(msg.chat_id_, msg.id_,'🗑┋ تم ازالة صورة المجموعه') 
end
return false  
end
if text == 'ضع وصف' or text == 'وضع وصف' then  
if Mod(msg) then
redis:setex('Globla:'..bot_id.."Set:Description" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_, 120, true)  
sendMessage(msg.chat_id_, msg.id_,'📥┋ ارسل الان الوصف')
end
return false  
end
if text == 'ضع ترحيب' or text == 'وضع ترحيب' then  
if Mod(msg) then
redis:setex('Globla:'..bot_id.."Welcome:Group" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_, 120, true)  
t  = '📨┋ ارسل لي الترحيب الان'
tt = '\n🌀┋ يمكنك اضافة مايلي !\n👤┋ دالة عرض الاسم »{`name`}\n📌┋ دالة عرض المعرف »{`user`}'
sendMessage(msg.chat_id_, msg.id_,t..tt) 
end
return false  
end
...
sendMessage(msg.chat_id_, msg.id_,'['..GetWelcome..']') 
return false  
end
...
sendMessage(msg.chat_id_, msg.id_,'☑┋ تم تفعيل ترحيب المجموعه') 
return false  
end
...
sendMessage(msg.chat_id_, msg.id_,'🚫┋ تم تعطيل ترحيب المجموعه') 
return false  
end
if text == 'مسح الترحيب' or text == 'حذف الترحيب' then 
if Mod(msg) then
redis:del('Globla:'..bot_id..'Get:Welcome:Group'..msg.chat_id_) 
sendMessage(msg.chat_id_, msg.id_,'🗑┋ تم ازالة ترحيب المجموعه') 
end
return false  
end
...
for k,v in pairs(list) do  
redis:del('Globla:'..bot_id.."Add:Filter:Rp1"..msg.sendMessageer_user_id_..msg.chat_id_)  
redis:del('Globla:'..bot_id.."Add:Filter:Rp2"..v..msg.chat_id_)  
redis:srem('Globla:'..bot_id.."List:Filter"..msg.chat_id_,v)  
end  
sendMessage(msg.chat_id_, msg.id_,"🗑┋ تم مسح قائمه المنع")  
end
...
sendMessage(msg.chat_id_, msg.id_,'📨┋ ارسل الكلمه لمنعها')  
redis:set('Globla:'..bot_id.."Add:Filter:Rp1"..msg.sendMessageer_user_id_..msg.chat_id_,"rep")  
return false  
end    
if text then   
local tsssst = redis:get('Globla:'..bot_id.."Add:Filter:Rp1"..msg.sendMessageer_user_id_..msg.chat_id_)  
if tsssst == 'rep' then   
sendMessage(msg.chat_id_, msg.id_,"📄┋ ارسل التحذير عند ارسال الكلمه")  
redis:set('Globla:'..bot_id.."Add:Filter:Rp1"..msg.sendMessageer_user_id_..msg.chat_id_,"repp")  
redis:set('Globla:'..bot_id.."filtr1:add:reply2"..msg.sendMessageer_user_id_..msg.chat_id_, text)  
redis:sadd('Globla:'..bot_id.."List:Filter"..msg.chat_id_,text)  
return false  end  
end
if text then  
local test = redis:get('Globla:'..bot_id.."Add:Filter:Rp1"..msg.sendMessageer_user_id_..msg.chat_id_)  
if test == 'repp' then  
sendMessage(msg.chat_id_, msg.id_,'📛┋تم منع الكلمه مع التحذير')  
redis:del('Globla:'..bot_id.."Add:Filter:Rp1"..msg.sendMessageer_user_id_..msg.chat_id_)  
local test = redis:get('Globla:'..bot_id.."filtr1:add:reply2"..msg.sendMessageer_user_id_..msg.chat_id_)  
if text then   
redis:set('Globla:'..bot_id.."Add:Filter:Rp2"..test..msg.chat_id_, text)  
end  
redis:del('Globla:'..bot_id.."filtr1:add:reply2"..msg.sendMessageer_user_id_..msg.chat_id_)  
return false  end  
end
...
sendMessage(msg.chat_id_, msg.id_,'📄┋ ارسل الكلمه الان')  
redis:set('Globla:'..bot_id.."Add:Filter:Rp1"..msg.sendMessageer_user_id_..msg.chat_id_,"reppp")  
return false  end
if text then 
local test = redis:get('Globla:'..bot_id.."Add:Filter:Rp1"..msg.sendMessageer_user_id_..msg.chat_id_)  
if test and test == 'reppp' then   
sendMessage(msg.chat_id_, msg.id_,"🌀┋ تم الغاء منعها ")  
redis:del('Globla:'..bot_id.."Add:Filter:Rp1"..msg.sendMessageer_user_id_..msg.chat_id_)  
...
if TEXT_SUDO then 
sendMessage(msg.chat_id_, msg.id_,TEXT_SUDO)
else
tdcli_function ({ID = "GetUser",user_id_ = SUDO},function(arg,result) 
local Name = '['..result.first_name_..'](tg://user?id='..result.id_..')'
sendMessageText(msg.chat_id_,Name,msg.id_/2097152/0.5,'md')
end,nil)
end
end
...
sendMessage(msg.chat_id_, msg.id_,'☑┋ تم حذف كليشه المطور')
end
if text == 'وضع كليشه المطور' or text == 'ضع كليشه المطور' and SudoBot(msg) then
redis:set('Globla:'..bot_id..'Set:TEXT_SUDO'..msg.chat_id_..':'..msg.sendMessageer_user_id_,true)
sendMessage(msg.chat_id_,msg.id_,'⌨┋ ارسل الكليشه الان')
return false
end
if text and redis:get('Globla:'..bot_id..'Set:TEXT_SUDO'..msg.chat_id_..':'..msg.sendMessageer_user_id_) then
if text == 'الغاء' then 
redis:del('Globla:'..bot_id..'Set:TEXT_SUDO'..msg.chat_id_..':'..msg.sendMessageer_user_id_)
sendMessage(msg.chat_id_,msg.id_,'☑┋تم الغاء حفظ كليشة المطور')
return false
end
redis:set('Globla:'..bot_id..'TEXT_SUDO',text)
redis:del('Globla:'..bot_id..'Set:TEXT_SUDO'..msg.chat_id_..':'..msg.sendMessageer_user_id_)
sendMessage(msg.chat_id_,msg.id_,'☑┋تم حفظ كليشة المطور')
return false
end
...
redis:setex('Globla:'..bot_id.."CHENG:ID"..msg.chat_id_..""..msg.sendMessageer_user_id_,240,true)  
local Text= [[
📃┋ارسل الان النص
🔰┋ يمكنك اضافه :
...
sendMessage(msg.chat_id_, msg.id_, '🗑┋ تم ازالة كليشة الايدي ')
end
return false  
end 

if redis:get('Globla:'..bot_id.."CHENG:ID"..msg.chat_id_..""..msg.sendMessageer_user_id_) then 
if text == 'الغاء' then 
sendMessage(msg.chat_id_, msg.id_,"🌀┋ تم الغاء تعين الايدي") 
redis:del('Globla:'..bot_id.."CHENG:ID"..msg.chat_id_..""..msg.sendMessageer_user_id_) 
return false  
end 
redis:del('Globla:'..bot_id.."CHENG:ID"..msg.chat_id_..""..msg.sendMessageer_user_id_) 
local CHENGER_ID = text:match("(.*)")  
redis:set('Globla:'..bot_id.."ALALMY:ID"..msg.chat_id_,CHENGER_ID)
sendMessage(msg.chat_id_, msg.id_,'☑┋ تم تعين الايدي')    
end
...
if (c - x) == 0 then
sendMessage(msg.chat_id_, msg.id_, "✖┋ لا توجد بوتات في المجموعه")
else
...
sendMessage(msg.chat_id_, msg.id_, text..a..f)
end
end,nil)
end
end,nil)
...
if redis:get('Globla:'..bot_id.."Set:Rules:" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_) then 
if text == 'الغاء' then 
sendMessage(msg.chat_id_, msg.id_, "🌀┋ تم الغاء حفظ القوانين") 
redis:del('Globla:'..bot_id.."Set:Rules:" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_)
return false  
end 
redis:set('Globla:'..bot_id.."Set:Rules:Group" .. msg.chat_id_,text) 
sendMessage(msg.chat_id_, msg.id_,"☑┋ تم حفظ قوانين المجموعه") 
redis:del('Globla:'..bot_id.."Set:Rules:" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_)
end  
...
redis:setex('Globla:'..bot_id.."Set:Rules:" .. msg.chat_id_ .. ":" .. msg.sendMessageer_user_id_, 600, true) 
sendMessage(msg.chat_id_,msg.id_,"📄┋ ارسل لي القوانين الان")  
end
end
if text == 'مسح القوانين' or text == 'حذف القوانين' then  
if Mod(msg) then
sendMessage(msg.chat_id_, msg.id_,"🗑┋ تم ازالة قوانين المجموعه")  
...
if Set_Rules then     
sendMessage(msg.chat_id_,msg.id_, Set_Rules)   
else      
sendMessage(msg.chat_id_, msg.id_,"🌀┋ لا توجد قوانين هنا")   
end    
end
--------------------------
if text and text:match("^تغير رد المطور (.*)$") and Manager(msg) then
local Teext = text:match("^تغير رد المطور (.*)$") 
redis:set('Globla:'..bot_id.."Sudo:Rd"..msg.chat_id_,Teext)
sendMessage(msg.chat_id_, msg.id_,"👥┋ تم تغير رد المطور الى » "..Teext)
end
if text and text:match("^تغير رد المنشئ الاساسي (.*)$") and Manager(msg) then
local Teext = text:match("^تغير رد المنشئ الاساسي (.*)$") 
redis:set('Globla:'..bot_id.."BasicConstructor:Rd"..msg.chat_id_,Teext)
sendMessage(msg.chat_id_, msg.id_,"👥┋ تم تغير رد المنشئ الاساسي الى » "..Teext)
end
if text and text:match("^تغير رد المنشئ (.*)$") and Manager(msg) then
local Teext = text:match("^تغير رد المنشئ (.*)$") 
redis:set('Globla:'..bot_id.."Constructor:Rd"..msg.chat_id_,Teext)
sendMessage(msg.chat_id_, msg.id_,"👥┋ تم تغير رد المنشئ الى » "..Teext)
end
if text and text:match("^تغير رد المدير (.*)$") and Manager(msg) then
local Teext = text:match("^تغير رد المدير (.*)$") 
redis:set('Globla:'..bot_id.."Manager:Rd"..msg.chat_id_,Teext) 
sendMessage(msg.chat_id_, msg.id_,"👥┋ تم تغير رد المدير الى » "..Teext)
end
if text and text:match("^تغير رد الادمن (.*)$") and Manager(msg) then
local Teext = text:match("^تغير رد الادمن (.*)$") 
redis:set('Globla:'..bot_id.."Mod:Rd"..msg.chat_id_,Teext)
sendMessage(msg.chat_id_, msg.id_,"👥┋ تم تغير رد الادمن الى » "..Teext)
end
if text and text:match("^تغير رد المميز (.*)$") and Manager(msg) then
local Teext = text:match("^تغير رد المميز (.*)$") 
redis:set('Globla:'..bot_id.."Special:Rd"..msg.chat_id_,Teext)
sendMessage(msg.chat_id_, msg.id_,"👥┋ تم تغير رد المميز الى » "..Teext)
end
if text and text:match("^تغير رد العضو (.*)$") and Manager(msg) then
local Teext = text:match("^تغير رد العضو (.*)$") 
redis:set('Globla:'..bot_id.."Memp:Rd"..msg.chat_id_,Teext)
sendMessage(msg.chat_id_, msg.id_,"👥┋ تم تغير رد العضو الى » "..Teext)
end
if text == 'حذف رد المطور' and Manager(msg) then
redis:del('Globla:'..bot_id.."Sudo:Rd"..msg.chat_id_)
sendMessage(msg.chat_id_, msg.id_,"☑┋تم حدف رد المطور")
end
if text == 'حذف رد المنشئ الاساسي' and Manager(msg) then
redis:del('Globla:'..bot_id.."BasicConstructor:Rd"..msg.chat_id_)
sendMessage(msg.chat_id_, msg.id_,"☑┋تم حذف رد المنشئ الاساسي ")
end
if text == 'حذف رد المنشئ' and Manager(msg) then
redis:del('Globla:'..bot_id.."Constructor:Rd"..msg.chat_id_)
sendMessage(msg.chat_id_, msg.id_,"☑┋تم حذف رد المنشئ ")
end
if text == 'حذف رد المدير' and Manager(msg) then
redis:del('Globla:'..bot_id.."Manager:Rd"..msg.chat_id_) 
sendMessage(msg.chat_id_, msg.id_,"☑┋تم حذف رد المدير ")
end
if text == 'حذف رد الادمن' and Manager(msg) then
redis:del('Globla:'..bot_id.."Mod:Rd"..msg.chat_id_)
sendMessage(msg.chat_id_, msg.id_,"☑┋تم حذف رد الادمن ")
end
if text == 'حذف رد المميز' and Manager(msg) then
redis:del('Globla:'..bot_id.."Special:Rd"..msg.chat_id_)
sendMessage(msg.chat_id_, msg.id_,"☑┋تم حذف رد المميز")
end
if text == 'حذف رد العضو' and Manager(msg) then
redis:del('Globla:'..bot_id.."Memp:Rd"..msg.chat_id_)
sendMessage(msg.chat_id_, msg.id_,"☑┋تم حذف رد العضو")
end
----------------------------------------------------------
...
sendMessage(msg.chat_id_, msg.id_,'☑┋تم فحص الحسابات المحذوفه وتم طردها من المجموعة')
end,nil)
end
end
----------------------------------------------------------
...
if #list == 0 then
sendMessage(msg.chat_id_, msg.id_,'🌀┋لا توجد صلاحيات مضافه')
return false
end
...
redis:set('Globla:'..bot_id.."Comd:New:rt"..msg.chat_id_..msg.sendMessageer_user_id_,ComdNew)  
redis:sadd('Globla:'..bot_id.."Coomds"..msg.chat_id_,ComdNew)  
redis:setex('Globla:'..bot_id.."Comd:New"..msg.chat_id_..""..msg.sendMessageer_user_id_,200,true)  
sendMessage(msg.chat_id_, msg.id_, "🗣 ┋ دز نـوع رتـبـه ?\n👨‍✈️ ┋ {عـضـو -- ممـيـز -- ادمـن -- مـديـر}") 
end
...
sendMessage(msg.chat_id_, msg.id_, "*🗑┋ تم مسح الصلاحيه *\n✓") 
end
if redis:get('Globla:'..bot_id.."Comd:New"..msg.chat_id_..""..msg.sendMessageer_user_id_) then 
if text and text:match("^الغاء$") then 
sendMessage(msg.chat_id_, msg.id_,"*🌀┋ تم الغاء الامر *\n✓") 
redis:del('Globla:'..bot_id.."Comd:New"..msg.chat_id_..""..msg.sendMessageer_user_id_) 
return false  
end 
if text == 'مدير' then
if not Constructor(msg) then
sendMessage(msg.chat_id_, msg.id_"👨‍✈️┋ يمكنك اضافه صلاحيات {ادمن - مميز - عضو} \n🗣┋ ارسل الصلاحيه مجددا\n") 
return false
end
end
if text == 'ادمن' then
if not Manager(msg) then 
sendMessage(msg.chat_id_, msg.id_,"👨‍✈️┋ يمكنك اضافه صلاحيات {مميز - عضو} \n🗣┋ ارسل الصلاحيه مجددا\n") 
return false
end
end
if text == 'مميز' then
if not Mod(msg) then
sendMessage(msg.chat_id_, msg.id_,"👨‍✈️┋ يمكنك اضافه صلاحيات {عضو} \n🗣┋ ارسل الصلاحيه مجددا\n") 
return false
end
end
if text == 'مدير' or text == 'ادمن' or text == 'مميز' or text == 'عضو' then
local textn = redis:get('Globla:'..bot_id.."Comd:New:rt"..msg.chat_id_..msg.sendMessageer_user_id_)  
redis:set('Globla:'..bot_id.."Comd:New:rt:bot:"..textn..msg.chat_id_,text)
sendMessage(msg.chat_id_, msg.id_, "☑ ┋ تـم اضـافـه الامـر √") 
redis:del('Globla:'..bot_id.."Comd:New"..msg.chat_id_..""..msg.sendMessageer_user_id_) 
return false  
end 
end
...
sendMessage(msg.chat_id_, msg.id_,'\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'..'\n📬┋ تم رفعه '..RTPA..' هنا\n')   
redis:set('Globla:'..bot_id.."Comd:New:rt:user:"..msg.chat_id_..result.sendMessageer_user_id_,RTPA) 
redis:sadd('Globla:'..bot_id..'Special:User'..msg.chat_id_,result.sendMessageer_user_id_)  
...
redis:srem('Globla:'..bot_id..'Special:User'..msg.chat_id_,result.sendMessageer_user_id_)  
redis:del('Globla:'..bot_id.."Comd:New:rt:user:"..msg.chat_id_..result.sendMessageer_user_id_)
elseif blakrt == 'ادمن' and Manager(msg) then 
sendMessage(msg.chat_id_, msg.id_,'\n👤┋المستخدم » ['..data.first_name_..'](t.me/'..(data.username_ or 'GLOBLA')..')'..'\n📬┋ تم تنزيله من '..RTPA..' هنا\n')   
redis:srem('Globla:'..bot_id..'Mod:User'..msg.chat_id_,result.sendMessageer_user_id_) 
redis:del('Globla:'..bot_id.."Comd:New:rt:user:"..msg.chat_id_..result.sendMessageer_user_id_)
...
info = '📌┋ المعرف خطأ'
sendMessage(msg.chat_id_, msg.id_,info)
end
end
...
sendMessage(msg.chat_id_, msg.id_,'🗑┋ تم مسح رسائلك جميعها'  )  
redis:del('Globla:'..bot_id..'Msg_User'..msg.chat_id_..':'..msg.sendMessageer_user_id_) 
end
if text == "رسايلي" or text == "رسائلي" or text == "msg" then 
sendMessage(msg.chat_id_, msg.id_,'📨┋ عدد رسائلك هنا » { '..redis:get('Globla:'..bot_id..'Msg_User'..msg.chat_id_..':'..msg.sendMessageer_user_id_)..'}' ) 
end 
----------------------------------------------------------
if text == 'تفعيل الاذاعه' and SudoBot(msg) then  
sendMessage(msg.chat_id_, msg.id_, '☑┋ تم تفعيل الاذاعه')
redis:del('Globla:'..bot_id..'Bc:Bots'..msg.chat_id_)
end
if text == 'تعطيل الاذاعه' and SudoBot(msg) then  
sendMessage(msg.chat_id_, msg.id_, '🚫┋ تم تعطيل الاذاعه')
redis:set('Globla:'..bot_id..'Bc:Bots'..msg.chat_id_, true)
end
----------------------------------------------------------
if text == 'تفعيل التواصل' and SudoBot(msg) then  
sendMessage(msg.chat_id_, msg.id_, '☑┋ تم تفعيل التواصل')
...
if num > 1000 then 
sendMessage(msg.chat_id_, msg.id_,'☑┋ يمكنك التنظيف ل1000 رساله كحد اقصى') 
return false  
end  
local msgm = msg.id_
for i=1,tonumber(num) do
...
sendMessage(msg.chat_id_,msg.id_,'☑┋ تم حذف {'..num..'}')  
end
----------------------------------------------------------
...
sendMessage(msg.chat_id_, msg.id_,'['..GLOBLA_Msg[math.random(#GLOBLA_Msg)]..']') 
return false
end
----------------------------------------------------------
if text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 and Sudo(msg) then 
if redis:get('Globla:'..bot_id..'Bc:Bots') and not SudoBot(msg) then 
sendMessage(msg.chat_id_, msg.id_,'☑┋ الاذاعه معطله من قبل المطور الاساسي')
return false
end
if AddChannel(msg.sendMessageer_user_id_) == false then
local textchuser = redis:get('Globla:'..bot_id..'text:ch:user')
...
if data.message_ == "CHAT_ADMIN_REQUIRED" then
sendMessage(msg.chat_id_,msg.id_,"🚸┋ ليست لدي صلاحية تغير اسم المجموعه")  
else
...
function start_function(extra, result, success)
if tonumber(SUDO) == tonumber(result.sendMessageer_user_id_) then
sendMessage(msg.chat_id_, msg.id_,"👨‍🔧┋ لا يمكنك تنزيل المطور الاساسي")
return false 
end
if redis:sismember('Globla:'..bot_id..'Sudo:User',result.sendMessageer_user_id_) then
dev = 'المطور ،' else dev = '' end
if redis:sismember('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_, result.sendMessageer_user_id_) then
crr = 'منشئ اساسي ،' else crr = '' end
if redis:sismember('Globla:'..bot_id..'Constructor'..msg.chat_id_, result.sendMessageer_user_id_) then
cr = 'منشئ ،' else cr = '' end
if redis:sismember('Globla:'..bot_id..'Manager'..msg.chat_id_, result.sendMessageer_user_id_) then
own = 'مدير ،' else own = '' end
if redis:sismember('Globla:'..bot_id..'Mod:User'..msg.chat_id_, result.sendMessageer_user_id_) then
mod = 'ادمن ،' else mod = '' end
if redis:sismember('Globla:'..bot_id..'Special:User'..msg.chat_id_, result.sendMessageer_user_id_) then
vip = 'مميز ،' else vip = ''
end
if Can_or_NotCan(result.sendMessageer_user_id_,msg.chat_id_) ~= false then
sendMessage(msg.chat_id_, msg.id_,"\n🔰┋ تم تنزيل الشخص من الرتب التاليه \n📥┋ { "..dev..''..crr..''..cr..''..own..''..mod..''..vip.." } \n")
else
sendMessage(msg.chat_id_, msg.id_,"\n✖┋ليس لديه رتب حتى يمكنني تنزيله \n")
end
if tonumber(SUDO) == tonumber(msg.sendMessageer_user_id_) then
redis:srem('Globla:'..bot_id..'Sudo:User', result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_,result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Constructor'..msg.chat_id_, result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Manager'..msg.chat_id_, result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Mod:User'..msg.chat_id_, result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Special:User'..msg.chat_id_, result.sendMessageer_user_id_)
elseif redis:sismember('Globla:'..bot_id..'Sudo:User',msg.sendMessageer_user_id_) then
redis:srem('Globla:'..bot_id..'Mod:User'..msg.chat_id_, result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Special:User'..msg.chat_id_, result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Manager'..msg.chat_id_, result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Constructor'..msg.chat_id_, result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_,result.sendMessageer_user_id_)
elseif redis:sismember('Globla:'..bot_id..'Basic:Constructor'..msg.chat_id_, msg.sendMessageer_user_id_) then
redis:srem('Globla:'..bot_id..'Mod:User'..msg.chat_id_, result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Special:User'..msg.chat_id_, result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Manager'..msg.chat_id_, result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Constructor'..msg.chat_id_, result.sendMessageer_user_id_)
elseif redis:sismember('Globla:'..bot_id..'Constructor'..msg.chat_id_, msg.sendMessageer_user_id_) then
redis:srem('Globla:'..bot_id..'Mod:User'..msg.chat_id_, result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Special:User'..msg.chat_id_, result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Manager'..msg.chat_id_, result.sendMessageer_user_id_)
elseif redis:sismember('Globla:'..bot_id..'Manager'..msg.chat_id_, msg.sendMessageer_user_id_) then
redis:srem('Globla:'..bot_id..'Mod:User'..msg.chat_id_, result.sendMessageer_user_id_)
redis:srem('Globla:'..bot_id..'Special:User'..msg.chat_id_, result.sendMessageer_user_id_)
end
end
...
sendMessage(msg.chat_id_, msg.id_,"📌┋ تم مسح ردود المطور")
end
...
local test = redis:get('Globla:'..bot_id..'Text:Sudo:Bot'..msg.sendMessageer_user_id_..':'..msg.chat_id_)
if redis:get('Globla:'..bot_id..'Set:Rd'..msg.sendMessageer_user_id_..':'..msg.chat_id_) == 'true1' then
redis:del('Globla:'..bot_id..'Set:Rd'..msg.sendMessageer_user_id_..':'..msg.chat_id_)
...
sendMessage(msg.chat_id_, msg.id_,'☑┋ تم حفظ الرد بنجاح')
return false  
end  
end
if text and text:match("^(.*)$") then
if redis:get('Globla:'..bot_id..'Set:Rd'..msg.sendMessageer_user_id_..':'..msg.chat_id_) == 'true' then
sendMessage(msg.chat_id_, msg.id_,'📥┋ ارسل الرد الذي تريده سواء كان { متحركه ‹› ملصق ‹› صوره ‹› اغنيه ‹› بصمه ‹› ملف ‹› فيديو }')
redis:set('Globla:'..bot_id..'Set:Rd'..msg.sendMessageer_user_id_..':'..msg.chat_id_, 'true1')
redis:set('Globla:'..bot_id..'Text:Sudo:Bot'..msg.sendMessageer_user_id_..':'..msg.chat_id_, text)
redis:sadd('Globla:'..bot_id..'List:Rd:Sudo', text)
return false end
end
if text and text:match("^(.*)$") then
if redis:get('Globla:'..bot_id..'Set:On'..msg.sendMessageer_user_id_..':'..msg.chat_id_) == 'true' then
sendMessage(msg.chat_id_, msg.id_,'🗑┋ تم ازالة الرد من قائمه ردود المطور')
list = {"Add:Rd:Sudo:Audio","Add:Rd:Sudo:File","Add:Rd:Sudo:Video","Add:Rd:Sudo:Photo","Add:Rd:Sudo:Text","Add:Rd:Sudo:stekr","Add:Rd:Sudo:vico","Add:Rd:Sudo:Gif"}
for k,v in pairs(list) do
redis:del('Globla:'..bot_id..v..text)
end
redis:del('Globla:'..bot_id..'Set:On'..msg.sendMessageer_user_id_..':'..msg.chat_id_)
...
sendMessage(msg.chat_id_, msg.id_,'📥┋ ارسل الكلمه التي تريد اضافتها')
redis:set('Globla:'..bot_id..'Set:Rd'..msg.sendMessageer_user_id_..':'..msg.chat_id_,true)
return false 
end
if text == 'حذف رد عام' and SudoBot(msg) then 
sendMessage(msg.chat_id_, msg.id_,'☑┋ ارسل الكلمه التي تريد اضافتها')
redis:set('Globla:'..bot_id..'Set:On'..msg.sendMessageer_user_id_..':'..msg.chat_id_,true)
return false 
end
if text and not redis:get('Globla:'..bot_id..'lock:Reply:Sudo'..msg.chat_id_) then
if not redis:sismember('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) then
...
------------------------------------------------------------------------
if text1 then 
sendMessage(msg.chat_id_, msg.id_,text1)
redis:sadd('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) 
end
if stekr then 
sendMessageSticker(msg.chat_id_, msg.id_, 0, 1, nil, stekr)   
redis:sadd('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) 
end
if veico then 
sendMessageVoice(msg.chat_id_, msg.id_, 0, 1, nil, veico)   
redis:sadd('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) 
end
if video then 
sendMessageVideo(msg.chat_id_, msg.id_, 0, 1, nil,video)
redis:sadd('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) 
end
if anemi then 
sendMessageDocument(msg.chat_id_, msg.id_, 0, 1, nil, anemi, '', nil)  
redis:sadd('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) 
end
if document then
sendMessageDocument(msg.chat_id_, msg.id_, 0, 1,nil, document)   
redis:sadd('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) 
end  
if audio then
sendMessageAudio(msg.chat_id_,msg.id_,audio)  
redis:sadd('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) 
end
if photo then
sendMessagePhoto(msg.chat_id_, msg.id_, 0, 1, nil,photo,'')
redis:sadd('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) 
end  
end
end
...
sendMessage(msg.chat_id_, msg.id_,"🗑┋ تم مسح ردود المدير")
end
...
if text or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.animation_ or msg.content_.audio_ or msg.content_.document_ or msg.content_.photo_ or msg.content_.video_ then  
local test = redis:get('Globla:'..bot_id..'Text:Manager'..msg.sendMessageer_user_id_..':'..msg.chat_id_..'')
if redis:get('Globla:'..bot_id..'Set:Manager:rd'..msg.sendMessageer_user_id_..':'..msg.chat_id_) == 'true1' then
redis:del('Globla:'..bot_id..'Set:Manager:rd'..msg.sendMessageer_user_id_..':'..msg.chat_id_)
if msg.content_.sticker_ then   
...
if text and text:match("^(.*)$") then
if redis:get('Globla:'..bot_id..'Set:Manager:rd'..msg.sendMessageer_user_id_..':'..msg.chat_id_) == 'true' then
sendMessage(msg.chat_id_, msg.id_,'📥┋ ارسل الرد الذي تريده سواء كان { متحركه ‹› ملصق ‹› صوره ‹› اغنيه ‹› بصمه ‹› ملف ‹› فيديو }')
redis:set('Globla:'..bot_id..'Set:Manager:rd'..msg.sendMessageer_user_id_..':'..msg.chat_id_,'true1')
redis:set('Globla:'..bot_id..'Text:Manager'..msg.sendMessageer_user_id_..':'..msg.chat_id_, text)
...
if text and text:match("^(.*)$") then
if redis:get('Globla:'..bot_id..'Set:Manager:rd'..msg.sendMessageer_user_id_..':'..msg.chat_id_..'') == 'true2' then
sendMessage(msg.chat_id_, msg.id_,'📌┋ تم ازالة الرد من قائمه ردود المدير')
redis:del('Globla:'..bot_id.."Add:Rd:Manager:Gif"..text..msg.chat_id_)   
redis:del('Globla:'..bot_id.."Add:Rd:Manager:Vico"..text..msg.chat_id_)   
redis:del('Globla:'..bot_id.."Add:Rd:Manager:Stekrs"..text..msg.chat_id_)     
redis:del('Globla:'..bot_id.."Add:Rd:Manager:Text"..text..msg.chat_id_)   
...
sendMessage(msg.chat_id_, msg.id_,'📝┋ ارسل الكلمه التري تريد اضافتها')
redis:set('Globla:'..bot_id..'Set:Manager:rd'..msg.sendMessageer_user_id_..':'..msg.chat_id_,true)
return false 
end
...
sendMessage(msg.chat_id_, msg.id_,'📝┋ ارسل الكلمه التري تريد حذفها')
redis:set('Globla:'..bot_id..'Set:Manager:rd'..msg.sendMessageer_user_id_..':'..msg.chat_id_,'true2')
return false 
end
...
if anemi then 
sendMessageDocument(msg.chat_id_, msg.id_, 0, 1,nil, anemi)   
redis:sadd('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) 
end
if document then
sendMessageDocument(msg.chat_id_, msg.id_, 0, 1,nil, document)   
...
if photo then
sendMessagePhoto(msg.chat_id_, msg.id_, 0, 1, nil,photo,photo_caption)
redis:sadd('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) 
end  
end
end
...
sendMessage(msg.chat_id_, msg.id_,'☑┋تم مغادرة المجموعه') 
redis:srem('Globla:'..bot_id..'Chek:Groups',msg.chat_id_)  
end
return false  
end
if text == 'بوت' then
Namebot = (redis:get('Globla:'..bot_id..'Name:Bot') or 'العالمي')
sendMessage(msg.chat_id_, msg.id_,'اسمي ['..Namebot..'] 😌👌') 
end
----------------------------------------------------------------------------
...
function start_function(extra, result, success)
tdcli_function ({ID = "GetUser",user_id_ = result.sendMessageer_user_id_},function(extra,data) 
if data.first_name_ == false then
sendMessage(msg.chat_id_, msg.id_,'👤┋ الحساب محذوف لا توجد معلوماته ')
return false
end
...
local Id = data.id_
local Msguser = tonumber(redis:get('Globla:'..bot_id..'Msg_User'..msg.chat_id_..':'..result.sendMessageer_user_id_) or 1) 
local Contact = tonumber(redis:get('Globla:'..bot_id..'Add:Contact'..msg.chat_id_..':'..result.sendMessageer_user_id_) or 0) 
local NUMPGAME = tonumber(redis:get('Globla:'..bot_id..'NUM:GAMES'..msg.chat_id_..result.sendMessageer_user_id_) or 0)
local edit = tonumber(redis:get('Globla:'..bot_id..'edits'..msg.chat_id_..result.sendMessageer_user_id_) or 0)
local rtp = Rutba(result.sendMessageer_user_id_,msg.chat_id_)
local iduser = result.sendMessageer_user_id_
sendMessage(msg.chat_id_, msg.id_,'🎟┋ ايديه » ('..iduser..')\n🎫┋ معرفه » ['..username..']\n👨‍🎓┋ رتبته » ('..rtp..')\n🗑┋ سحكاته » ('..edit..')\n🤹‍♂️┋ نقاطه » ('..NUMPGAME..')\n📞┋ جهاته » ('..Contact..')\n📨┋ رسائله » ('..Msguser..')')
end,nil)
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, start_function, nil)
end
...
sendMessage(msg.chat_id_, msg.id_,'✖┋ المعرف غير صحيح ')
end
end
tdcli_function ({ID = "SearchPublicChat",username_ = username}, start_function, nil)
end
...
text = 'الملصق'
sendMessage(msg.chat_id_, msg.id_,textt..'( '..text..' ) عذراً لن تتمكن من ارسالها مجددا')  
return false
end
...
text = 'الملصق'
sendMessage(msg.chat_id_, msg.id_,textt..'( '..text..' ) بنجاح يمكنهم الارسال الان')  
return false
end
if b.content_.ID == "MessagePhoto" then
local photo = b.content_.photo_.id_
...
sendMessage(msg.chat_id_, msg.id_,'📛┋ تم مسح قائمه منع المتحركات')  
end
if text == 'مسح قائمه منع الصور' and Manager(msg) then     
redis:del('Globla:'..bot_id.."filterphoto"..msg.chat_id_)
sendMessage(msg.chat_id_, msg.id_,'📛┋ تم مسح قائمه منع الصور')  
end
if text == 'مسح قائمه منع الملصقات' and Manager(msg) then     
redis:del('Globla:'..bot_id.."filtersteckr"..msg.chat_id_)
sendMessage(msg.chat_id_, msg.id_,'📛┋ تم مسح قائمه منع الملصقات')  
end
----------------------------------------------------------------------------
if text == 'رتبتي' then
local rtp = Rutba(msg.sendMessageer_user_id_,msg.chat_id_)
sendMessage(msg.chat_id_, msg.id_,'👨‍🎓┋ رتبتك في البوت » '..rtp)
end
----------------------------------------------------------------------------
if text == 'ايديي' then
sendMessage(msg.chat_id_, msg.id_,'🎟┋ ايديك » '..msg.sendMessageer_user_id_)
end
----------------------------------------------
...
local iduser = result.sendMessageer_user_id_
sendMessage(msg.chat_id_, msg.id_,'🎟┋ الايدي » ('..iduser..')\n🎫┋ المعرف » ('..username..')\n👨‍🎓┋ الرتبه » ('..rtp..')\n🕵🏻️‍♀️┋ نوع الكشف » بالرد')
end,nil)
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, start_function, nil)
end
...
if msg.can_be_deleted_ == false then 
sendMessage(msg.chat_id_,msg.id_,"✖┋ البوت ليس ادمن هنا \n") 
return false  
end 
...
sendMessage(msg.chat_id_, msg.id_, zain) 
end,nil)
end,nil)
end 
----------------------------------------------------------------------------
...
if not redis:get('Globla:'..bot_id..'Cick:Me'..msg.chat_id_) then
if Can_or_NotCan(msg.sendMessageer_user_id_, msg.chat_id_) == true then
sendMessage(msg.chat_id_, msg.id_, '\n✖┋ عذرا لا يمكنني طرد ( '..Rutba(msg.sendMessageer_user_id_,msg.chat_id_)..' )')
return false
end
tdcli_function({ID="ChangeChatMemberStatus",chat_id_=msg.chat_id_,user_id_=msg.sendMessageer_user_id_,status_={ID="ChatMemberStatusKicked"},},function(arg,data) 
if (data and data.code_ and data.code_ == 400 and data.message_ == "CHAT_ADMIN_REQUIRED") then 
sendMessage(msg.chat_id_, msg.id_,'✖┋ ليس لدي صلاحية حظر المستخدمين يرجى تفعيلها !') 
return false  
end
...
sendMessage(msg.chat_id_, msg.id_,'✖┋ عذرا لا يمكنني طرد ادمنية المجموعه') 
return false  
end
if data and data.ID and data.ID == 'Ok' then
sendMessage(msg.chat_id_, msg.id_,'🚷┋ تم طردك من المجموعه ') 
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = msg.chat_id_, user_id_ = msg.sendMessageer_user_id_, status_ = { ID = "ChatMemberStatusLeft" },},function(arg,ban) end,nil)   
return false
end
end,nil)   
else
sendMessage(msg.chat_id_, msg.id_,'✖┋ امر اطردني تم تعطيله من قبل المدراء ') 
end
end
----------------------------------------------------------------------------
...
sendMessage(msg.chat_id_, msg.id_,'✖┋ المعرف خطأ ') 
return false  
end
if result and result.type_ and result.type_.channel_ and result.type_.channel_.ID == "Channel" then
sendMessage(msg.chat_id_, msg.id_,'✖┋ لا اسطيع صيح معرفات القنوات') 
return false  
end
if result.type_.user_.type_.ID == "UserTypeBot" then
sendMessage(msg.chat_id_, msg.id_,'✖┋ لا اسطيع صيح معرفات البوتات') 
return false  
end
if result and result.type_ and result.type_.channel_ and result.type_.channel_.is_supergroup_ == true then
sendMessage(msg.chat_id_, msg.id_,'✖┋ لا اسطيع صيح معرفات المجموعات') 
return false  
end
if result.id_ then
sendMessage(msg.chat_id_, msg.id_,'👤┋ تعال حبي يصيحونك بل كروب [@'..username..']') 
return false
end
end
tdcli_function ({ID = "SearchPublicChat",username_ = username}, start_function, nil)
else
sendMessage(msg.chat_id_, msg.id_,'✖┋ امر صيح تم تعطيله من قبل المدراء ') 
end
return false
end
----------------------------------------------------------------------------
...
sendMessage(msg.chat_id_, msg.id_,'🔅┋ انت منشئ المجموعه ') 
return false
end
local Added_Me = redis:get('Globla:'..bot_id.."Who:Added:Me"..msg.chat_id_..':'..msg.sendMessageer_user_id_)
if Added_Me then 
tdcli_function ({ID = "GetUser",user_id_ = Added_Me},function(extra,result,success)
local Name = '['..result.first_name_..'](tg://user?id='..result.id_..')'
Text = '👤┋ الشخص الذي قام باضافتك هو » '..Name
sendMessageText(msg.chat_id_,Text,msg.id_/2097152/0.5,'md')
end,nil)
else
sendMessage(msg.chat_id_, msg.id_,'📎┋ انت دخلت عبر الرابط') 
end
end,nil)
else
sendMessage(msg.chat_id_, msg.id_,'👥┋ امر منو ضافني تم تعطيله من قبل المدراء ') 
end
end
----------------------------------------------------------------------------
if text == 'تفعيل ضافني' and Manager(msg) then  
sendMessage(msg.chat_id_, msg.id_, '☑┋ تم تفعيل ضافني')
redis:del('Globla:'..bot_id..'Added:Me'..msg.chat_id_)
end
if text == 'تعطيل ضافني' and Manager(msg) then  
sendMessage(msg.chat_id_, msg.id_, '🚫┋ تم تعطيل ضافني')
redis:set('Globla:'..bot_id..'Added:Me'..msg.chat_id_, true)
end
----------------------------------------------------------------------------
if text == 'تفعيل صيح' and Manager(msg) then  
sendMessage(msg.chat_id_, msg.id_, '☑┋ تم تفعيل صيح')
redis:del('Globla:'..bot_id..'Seh:User'..msg.chat_id_)
end
if text == 'تعطيل صيح' and Manager(msg) then  
sendMessage(msg.chat_id_, msg.id_, '🚫┋ تم تعطيل صيح')
redis:set('Globla:'..bot_id..'Seh:User'..msg.chat_id_, true)
end
----------------------------------------------------------------------------
if text == 'تفعيل اطردني' and Manager(msg) then  
sendMessage(msg.chat_id_, msg.id_, '☑┋ تم تفعيل اطردني')
redis:del('Globla:'..bot_id..'Cick:Me'..msg.chat_id_)
end
if text == 'تعطيل اطردني' and Manager(msg) then  
sendMessage(msg.chat_id_, msg.id_, '🚫┋ تم تعطيل اطردني')
...
if not redis:sismember('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) then
redis:sadd('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) 
tdcli_function ({ID = "GetChatMember",chat_id_ = msg.chat_id_,user_id_ = msg.sendMessageer_user_id_},function(arg,da)  tdcli_function ({ ID = "SendChatAction",  chat_id_ = msg.sendMessageer_user_id_, action_ = {  ID = "SendMessageTypingAction", progress_ = 100}  },function(arg,ta)  tdcli_function ({ID = "GetUser",user_id_ = msg.sendMessageer_user_id_},function(extra,result,success)  tdcli_function ({ID = "GetUserProfilePhotos",user_id_ = msg.sendMessageer_user_id_,offset_ = 0,limit_ = 1},function(extra,zain,success) 
if da.status_.ID == "ChatMemberStatusCreator" then 
rtpa = 'منشئ'
elseif da.status_.ID == "ChatMemberStatusEditor" then 
rtpa = 'مشرف' 
...
local Msguser = tonumber(redis:get('Globla:'..bot_id..'Msg_User'..msg.chat_id_..':'..msg.sendMessageer_user_id_) or 1) 
local Contact = tonumber(redis:get('Globla:'..bot_id..'Add:Contact'..msg.chat_id_..':'..msg.sendMessageer_user_id_) or 0) 
local NUMPGAME = tonumber(redis:get('Globla:'..bot_id..'NUM:GAMES'..msg.chat_id_..msg.sendMessageer_user_id_) or 0)
local rtp = Rutba(msg.sendMessageer_user_id_,msg.chat_id_)
local username = '@'..(result.username_ or 'لا يوجد')
local iduser = msg.sendMessageer_user_id_
local edit = tonumber(redis:get('Globla:'..bot_id..'edits'..msg.chat_id_..msg.sendMessageer_user_id_) or 0)
...
sendMessagePhoto(msg.chat_id_, msg.id_, 0, 1, nil, zain.photos_[0].sizes_[1].photo_.persistent_id_,get_id_text)       
else 
if result.status_.ID == "UserStatusEmpty" and result.profile_photo_ == false then
sendMessage(msg.chat_id_, msg.id_,'['..get_id_text..']')   
else
...
if result.status_.ID == "UserStatusRecently" and result.profile_photo_ ~= false then
sendMessagePhoto(msg.chat_id_, msg.id_, 0, 1, nil, zain.photos_[0].sizes_[1].photo_.persistent_id_,'🎥┋ '..rdphoto..'\n🎟┋ ايديك • ('..msg.sendMessageer_user_id_..')\n🎭┋ معرفك • '..username..'\n📌┋موقعك • '..Rutba(msg.sendMessageer_user_id_,msg.chat_id_)..'\n🏞┋ صورك • '..photps..'\n⏱┋ تفاعلك • '..Total_Msg(Msguser)..'\n💌┋ رسائلك • {'..Msguser..'}\n📧┋ سحكاتك • {'..edit..'}\n🤹‍♂️┋ نقاطك • {'..NUMPGAME..'}\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉')   
else 
if result.status_.ID == "UserStatusEmpty" and result.profile_photo_ == false then
sendMessage(msg.chat_id_, msg.id_,'[\n🎟┋ ايديك • ('..msg.sendMessageer_user_id_..')\n🎭┋ معرفك • '..username..'\n📌┋موقعك • '..Rutba(msg.sendMessageer_user_id_,msg.chat_id_)..'\n🏞┋ صورك • '..photps..'\n⏱┋ تفاعلك • '..Total_Msg(Msguser)..'\n💌┋ رسائلك • {'..Msguser..'}\n📧┋ سحكاتك • {'..edit..']}\n🤹‍♂️┋ نقاطك • {'..NUMPGAME..'}\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉')   
else
...
local Num = tonumber(redis:get('Globla:'..bot_id..'edits'..msg.chat_id_..msg.sendMessageer_user_id_) or 0)
if Num == 0 then 
...
sendMessage(msg.chat_id_, msg.id_,'🗑┋ تم مسح سحكاتك'  )  
redis:del('Globla:'..bot_id..'edits'..msg.chat_id_..msg.sendMessageer_user_id_)
end
----------------------------------------------------------------------------
if text == "مسح جهاتي" or text == "حذف جهاتي" then  
sendMessage(msg.chat_id_, msg.id_,'🗑┋ تم مسح جهاتك'  )  
redis:del('Globla:'..bot_id..'Add:Contact'..msg.chat_id_..':'..msg.sendMessageer_user_id_)
end
----------------------------------------------------------------------------
if text == 'جهاتي' or text == 'شكد ضفت' then 
local Num = tonumber(redis:get('Globla:'..bot_id..'Add:Contact'..msg.chat_id_..':'..msg.sendMessageer_user_id_) or 0) 
if Num == 0 then 
...
sendMessage(msg.chat_id_, msg.id_, '☑┋ تم تفعيل الردود')
redis:del('Globla:'..bot_id..'lock:add'..msg.chat_id_)
end
if text == 'تعطيل الردود' and Manager(msg) then  
sendMessage(msg.chat_id_, msg.id_, '🚫┋ تم تعطيل الردود')
...
sendMessage(msg.chat_id_, msg.id_, ''..texting[math.random(#texting)]..'')
end
end
-------------------------------
...
sendMessage(msg.chat_id_, msg.id_,'🔰┋ اسرع واحد يدز هذا السمايل ? » {`'..SM..'`}')
return false
end
end
...
sendMessage(msg.chat_id_, msg.id_,'🎁┋ الف مبروك لقد فزت \n♻┋ للعب مره اخره ارسل »{ سمايل , سمايلات }')
redis:incrby('Globla:'..bot_id..'NUM:GAMES'..msg.chat_id_..msg.sendMessageer_user_id_, 1)  
end
...
sendMessage(msg.chat_id_, msg.id_,'🔰┋ اسرع واحد يرتبها » {'..name..'}')
return false
end
end
------------------------------------------------------------------------
...
sendMessage(msg.chat_id_, msg.id_,'🔰┋ اسرع واحد يحل الحزوره ↓\n {'..name..'}')
return false
end
end
------------------------------------------------------------------------
...
sendMessage(msg.chat_id_, msg.id_,'🔰┋ اسرع واحد يدز معنى السمايل » {'..name..'}')
return false
end
end
------------------------------------------------------------------------
...
sendMessage(msg.chat_id_, msg.id_,'🔰┋ اسرع واحد يدز العكس » {'..name..'}')
return false
end
end
------------------------------------------------------------------------
...
if redis:get('Globla:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 20 then
sendMessage(msg.chat_id_, msg.id_,"📬┋ عذرآ لا يمكنك تخمين عدد اكبر من ال { 20 } خمن رقم ما بين ال{ 1 و 20 }\n")
return false  end 
local GETNUM = redis:get('Globla:'..bot_id.."GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
redis:del('Globla:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sendMessageer_user_id_)
redis:del('Globla:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)   
redis:incrby('Globla:'..bot_id..'NUM:GAMES'..msg.chat_id_..msg.sendMessageer_user_id_,5)  
sendMessage(msg.chat_id_, msg.id_,'🔖┋ مبروك فزت ويانه وخمنت الرقم الصحيح\n🚸┋ تم اضافة { 5 } من النقاط \n')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
redis:incrby('Globla:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sendMessageer_user_id_,1)
if tonumber(redis:get('Globla:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sendMessageer_user_id_)) >= 3 then
redis:del('Globla:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sendMessageer_user_id_)
redis:del('Globla:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)   
sendMessage(msg.chat_id_, msg.id_,'📮┋ اوبس لقد خسرت في اللعبه \n📬┋ حظآ اوفر في المره القادمه \n🔰┋ كان الرقم الذي تم تخمينه { '..GETNUM..' }')
else
sendMessage(msg.chat_id_, msg.id_,'📛┋ اوبس تخمينك خطأ \n📌┋ ارسل رقم تخمنه مره اخرى ')
end
end
end
end
...
redis:setex('Globla:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_, 100, true)  
return false  
end
end

if redis:get('Globla:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 6 then
sendMessage(msg.chat_id_, msg.id_,"📬┋ عذرا لا يوجد سواء { 6 } اختيارات فقط ارسل اختيارك مره اخرى\n")
return false  end 
local GETNUM = redis:get('Globla:'..bot_id.."Games:Bat"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
redis:del('Globla:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)   
sendMessage(msg.chat_id_, msg.id_,'📮┋ مبروك فزت وطلعت المحيبس بل ايد رقم { '..NUM..' }\n🎊┋ لقد حصلت على { 3 }من نقاط يمكنك استبدالهن برسائل ')
redis:incrby('Globla:'..bot_id..'NUM:GAMES'..msg.chat_id_..msg.sendMessageer_user_id_,3)  
elseif tonumber(NUM) ~= tonumber(GETNUM) then
redis:del('Globla:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)   
sendMessage(msg.chat_id_, msg.id_,'📮┋ للاسف لقد خسرت \n📬┋ المحيبس بل ايد رقم { '..GETNUM..' }\n💥┋ حاول مره اخرى للعثور على المحيبس')
end
end
end
...
sendMessage(msg.chat_id_, msg.id_,TEST)
redis:setex('Globla:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_, 100, true)  
return false  
end
end
...
name = string.gsub(name,'⌛️','⏳⏳⏳⏳⏳⏳⌛️⏳⏳')
name = string.gsub(name,'📅','📆📆📆📆📆??📅📆📆')
sendMessage(msg.chat_id_, msg.id_,'🔰┋ اسرع واحد يدز الاختلاف » {'..name..'}')
return false
end
end
------------------------------------------------------------------------
...
sendMessage(msg.chat_id_, msg.id_,'🔰┋ اسرع واحد يكمل المثل » {'..name..'}')
return false
end
end
------------------------------------------------------------------------
...
if tonumber(redis:get('Globla:'..bot_id..'NUM:GAMES'..msg.chat_id_..msg.sendMessageer_user_id_)) == tonumber(0) then
sendMessage(msg.chat_id_,msg.id_,'✖┋ ليس لديك نقاط من الالعاب \n📬┋ اذا كنت تريد ربح النقاط \n📌┋ ارسل الالعاب وابدأ اللعب ! ') 
else
local NUM_GAMES = redis:get('Globla:'..bot_id..'NUM:GAMES'..msg.chat_id_..msg.sendMessageer_user_id_)
if tonumber(NUMPY) > tonumber(NUM_GAMES) then
sendMessage(msg.chat_id_,msg.id_,'\n✖┋ ليس لديك نقاط بهذا العدد \n📬┋ لزيادة نقاطك في اللعبه \n📌┋ ارسل الالعاب وابدأ اللعب !') 
return false 
end
local NUMNKO = (NUMPY * 50)
redis:decrby('Globla:'..bot_id..'NUM:GAMES'..msg.chat_id_..msg.sendMessageer_user_id_,NUMPY)  
redis:incrby('Globla:'..bot_id..'Msg_User'..msg.chat_id_..':'..msg.sendMessageer_user_id_,NUMNKO)  
sendMessage(msg.chat_id_,msg.id_,'🔅┋ تم خصم *» { '..NUMPY..' }* من نقاطك \n📨┋ وتم اضافة* » { '..(NUMPY * 50)..' } رساله الى رسالك *')
end 
return false 
end
...
if redis:get('Globla:'..bot_id.."numadd:user" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_) then 
if text and text:match("^الغاء$") then 
redis:del('Globla:'..bot_id..'id:user'..msg.chat_id_)  
sendMessage(msg.chat_id_, msg.id_, "☑┋ تم الغاء الامر ") 
redis:del('Globla:'..bot_id.."numadd:user" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)  
return false  
end 
redis:del('Globla:'..bot_id.."numadd:user" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)  
...
sendMessage(msg.chat_id_, msg.id_,"📥┋ تم اضافة له {"..numadded..'} من الرسائل')  
end
------------------------------------------------------------------------
if redis:get('Globla:'..bot_id.."gemadd:user" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_) then 
if text and text:match("^الغاء$") then 
redis:del('Globla:'..bot_id..'idgem:user'..msg.chat_id_)  
sendMessage(msg.chat_id_, msg.id_, "☑┋ تم الغاء الامر ") 
redis:del('Globla:'..bot_id.."gemadd:user" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)  
return false  
end 
redis:del('Globla:'..bot_id.."gemadd:user" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_)  
local numadded = string.match(text, "(%d+)") 
...
sendMessage(msg.chat_id_, msg.id_,  1, "📥┋ تم اضافة له {"..numadded..'} من النقاط', 1, 'md')  
end
------------------------------------------------------------
if text and text:match("^اضف رسائل (%d+)$") and msg.reply_to_message_id_ == 0 and Constructor(msg) then    
zain = text:match("^اضف رسائل (%d+)$")
redis:set('Globla:'..bot_id..'id:user'..msg.chat_id_,zain)  
redis:setex('Globla:'..bot_id.."numadd:user" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_, 120, true)  
sendMessage(msg.chat_id_, msg.id_, '📨┋ ارسل لي عدد الرسائل الان') 
return false
end
------------------------------------------------------------------------
...
redis:setex('Globla:'..bot_id.."gemadd:user" .. msg.chat_id_ .. "" .. msg.sendMessageer_user_id_, 120, true)  
sendMessage(msg.chat_id_, msg.id_, '🤹‍♂️┋ ارسل لي عدد النقاط الان') 
return false
end
------------------------------------------------------------------------
if text and text:match("^اضف نقاط (%d+)$") and msg.reply_to_message_id_ ~= 0 and Constructor(msg) then
...
redis:incrby('Globla:'..bot_id..'NUM:GAMES'..msg.chat_id_..result.sendMessageer_user_id_,Num)  
sendMessage(msg.chat_id_, msg.id_,"📥┋ تم اضافة له {"..Num..'} من النقاط')  
end
...
function reply(extra, result, success)
redis:del('Globla:'..bot_id..'Msg_User'..msg.chat_id_..':'..result.sendMessageer_user_id_) 
redis:incrby('Globla:'..bot_id..'Msg_User'..msg.chat_id_..':'..result.sendMessageer_user_id_,Num)  
...
sendMessage(msg.chat_id_, msg.id_,Text_Games) 
end
----------------------------------------------------------------------------
if text == 'الاوامر' then
if not Mod(msg) then
sendMessage(msg.chat_id_, msg.id_,'👥┋للأدمنيـــــــه فقــــــــط') 
return false
end
if AddChannel(msg.sendMessageer_user_id_) == false then
local textchuser = redis:get('Globla:'..bot_id..'text:ch:user')
...
sendMessage(msg.chat_id_, msg.id_,'👥┋ عذراً عليك الاشتراك في القناة\n📡┋قناة الاشتراك ['..redis:get('Globla:'..bot_id..'add:ch:username')..']')
end
return false
end
Text = [[
...
print(AddChannel(msg.sendMessageer_user_id_))
if AddChannel(msg.sendMessageer_user_id_) == false then
local textchuser = redis:get('Globla:'..bot_id..'text:ch:user')
if textchuser then
sendMessage(msg.chat_id_, msg.id_,'['..textchuser..']')
...
if text == 'م4' then
if not Manager(msg) then
sendMessage(msg.chat_id_, msg.id_,'👥┋للمدراء فقــــــــط') 
return false
end
if AddChannel(msg.sendMessageer_user_id_) == false then
local textchuser = redis:get('Globla:'..bot_id..'text:ch:user')
...
if text == 'م5' then
if not Constructor(msg) then
sendMessage(msg.chat_id_, msg.id_,'👥┋للمنشئين فقــــــــط') 
return false
end
if AddChannel(msg.sendMessageer_user_id_) == false then
local textchuser = redis:get('Globla:'..bot_id..'text:ch:user')
...
sendMessage(msg.chat_id_, msg.id_,'👨‍🔧┋للمطور الاساسي فقــــــــط') 
return false
end
if AddChannel(msg.sendMessageer_user_id_) == false then
local textchuser = redis:get('Globla:'..bot_id..'text:ch:user')
...
if tonumber(msg.sendMessageer_user_id_) == tonumber(bot_id) then
return false
end
--------------------------------------------------------------------------------------------------------------
if text and not redis:sismember('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_,text) then
redis:del('Globla:'..bot_id..'Spam:Texting'..msg.sendMessageer_user_id_) 
end
--------------------------------------------------------------------------------------------------------------
if text then
local NewCmmd = redis:get('Globla:'..bot_id.."Set:Cmd:Group:New1"..msg.chat_id_..':'..data.message_.content_.text_)
if NewCmmd then
data.message_.content_.text_ = (NewCmmd or data.message_.content_.text_)
end
end
if (text and text == "تعطيل اوامر التحشيش") then 
sendMessage(msg.chat_id_, msg.id_, '🚫┋تم تعطيل اوامر التحشيش')
redis:set('Globla:'..bot_id.."Fun_Bots:"..msg.chat_id_,"true")
end
if (text and text == "تفعيل اوامر التحشيش") then 
sendMessage(msg.chat_id_, msg.id_, '☑┋تم تفعيل اوامر التحشيش')
...
sendMessage(msg.chat_id_, result.id_,''..Fun[math.random(#Fun)]..'')   
end   
...
function FunBot(extra, result, success)
 if tonumber(result.sendMessageer_user_id_) == tonumber(SUDO) then
sendMessage(msg.chat_id_, msg.id_, 'انجب لك هذا مطوري العشق 😌💋')
return false  end
if tonumber(result.sendMessageer_user_id_) == tonumber(bot_id) then
sendMessage(msg.chat_id_, msg.id_, 'لك مگدر اهين نفسي 😞😂')
return false  end
...
if SudoBot(msg) then  
sendMessage(msg.chat_id_,msg.id_,"حاضر مو تدلل حضره المطور  😇 ")
elseif Sudo(msg) then 
sendMessage(msg.chat_id_,msg.id_,"حاضر مو تدلل مطوري 😋")
elseif Constructor(msg) then 
sendMessage(msg.chat_id_,msg.id_,"ع راسي تدلل انته المنشئ تاج راسي 😌")
elseif Manager(msg) then 
sendMessage(msg.chat_id_,msg.id_,"لخاطرك راح اسكت لان مدير وع راسي  😌")
elseif Mod(msg) then 
sendMessage(msg.chat_id_,msg.id_,"فوك مامصعدك ادمن ؟؟ انته انجب 😏") 
elseif Special(msg) then 
sendMessage(msg.chat_id_,msg.id_,"فوك مامصعدك مميز ؟ وتكمز 🤨")
else 
sendMessage(msg.chat_id_,msg.id_,"انجب انته لاتندفر 😏")
end 
end
-------------------------------------------------------------------------------------------------------------- 
if text and redis:get('Globla:'..bot_id.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sendMessageer_user_id_) == "true" then
local NewCmmd = redis:get('Globla:'..bot_id.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text)
if NewCmmd then
redis:del('Globla:'..bot_id.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text)
redis:del('Globla:'..bot_id.."Set:Cmd:Group:New"..msg.chat_id_)
redis:srem('Globla:'..bot_id.."List:Cmd:Group:New"..msg.chat_id_,text)
sendMessage(msg.chat_id_, msg.id_,"🗑┋تم ازالة الامر من المجموعه")  
else
sendMessage(msg.chat_id_, msg.id_,"✖┋لا يوجد امر بهاذا الاسم تاكد من الامر واعد المحاوله")  
end
redis:del('Globla:'..bot_id.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sendMessageer_user_id_)
return false
end
---------------
if text then
...
--------------------------------------------------------------------------------------------------------------
if msg.sendMessageer_user_id_ and Muted_User(msg.chat_id_,msg.sendMessageer_user_id_) then 
DeleteMessage(msg.chat_id_, {[0] = msg.id_})  
return false  
end
--------------------------------------------------------------------------------------------------------------
if msg.sendMessageer_user_id_ and Ban_User(msg.chat_id_,msg.sendMessageer_user_id_) then 
Group_Kick(msg.chat_id_,msg.sendMessageer_user_id_) 
DeleteMessage(msg.chat_id_, {[0] = msg.id_}) 
return false  
end
...
--------------------------------------------------------------------------------------------------------------
if msg.sendMessageer_user_id_ and GBan_User(msg.sendMessageer_user_id_) then 
Group_Kick(msg.chat_id_,msg.sendMessageer_user_id_) 
DeleteMessage(msg.chat_id_, {[0] = msg.id_}) 
return false 
end
...
--------------------------------------------------------------------------------------------------------------
if msg.content_.ID == "MessageChatAddMembers" then  
redis:set('Globla:'..bot_id.."Who:Added:Me"..msg.chat_id_..':'..msg.content_.members_[0].id_,msg.sendMessageer_user_id_)
local mem_id = msg.content_.members_  
local Bots = redis:get('Globla:'..bot_id.."lock:Bot:kick"..msg.chat_id_) 
for i=0,#mem_id do  
if msg.content_.members_[i].type_.ID == "UserTypeBot" and not Mod(msg) and Bots == "kick" then   
https.request("https://api.telegram.org/bot"..token.."/kickChatMember?chat_id="..msg.chat_id_.."&user_id="..msg.sendMessageer_user_id_)
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sendMessageer_user_id_},function(arg,data) 
if data.username_ then
redis:set('Globla:'..bot_id..'user:Name'..msg.sendMessageer_user_id_,(data.username_))
end
--------------------------------------------------------------------------------------------------------------
local Getredis = redis:get('Globla:'..bot_id.."Chen:Photo"..msg.sendMessageer_user_id_)
...
sendMessage(msg.chat_id_,msg.id_,zain[math.random(#zain)])
redis:set('Globla:'..bot_id.."Chen:Photo"..msg.sendMessageer_user_id_, data.profile_photo_.id_) 
return false
end
end
end,nil)   
...
redis:incr('Globla:'..bot_id..'edits'..result.chat_id_..result.sendMessageer_user_id_)
...
sendMessage(msg.chat_id_,0,'⚠️┋ تم التعديل على الميديا \n\n👤┋ الشخص الي قام بالتعديل\n➺➺➺ •⊱{ '..users..' }⊰•') 
end,nil)
...
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
redis:srem('Globla:'..bot_id..'Chek:Groups',v)  
...
redis:sadd('Globla:'..bot_id..'Chek:Groups',v)  
end 
end,nil)
end
end -- end new msg
